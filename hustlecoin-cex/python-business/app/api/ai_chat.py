...
def _get_config(db: Session, site: str) -> AiConfig | None:
    ...
    # The admin assistant has its own conversation/FAQ namespace, but older
    # installations only configured the user-facing ``coin`` provider.  Use
    # that configured provider as a compatibility fallback so enabling AI on
    # the coin site also makes the management assistant usable.  An explicit
    # enabled coinadmin config always wins, preserving per-site overrides.
    if site == "coinadmin" and (not cfg or not cfg.is_enabled or not cfg.api_key):
        coin_cfg = db.query(AiConfig).filter(AiConfig.site == "coin").first()
        if coin_cfg and coin_cfg.is_enabled and coin_cfg.api_key:
            return coin_cfg
    if not cfg:
...
def _build_faq_context(db: Session, site: str) -> str:
    # FAQ content is tenant scoped.  Do not inject another product's
    # operational instructions into this site's assistant prompt.
    faqs = (
        db.query(AiFaq)
        .filter(AiFaq.site == site, AiFaq.is_active == True)
        .order_by(AiFaq.sort_order)
        .all()
    )
    if not faqs:
        return ""
...
async def chat(req: ChatRequest, request: Request, db: Session = Depends(get_db)):
    ...
    faq_ctx = _build_faq_context(db, req.site)
