...
@router.get("/faq")
def list_faq(request: Request, db: Session = Depends(get_db), site: str = Query("default")):
    require_admin(request)
    try:
        items = db.query(AiFaq).filter(AiFaq.site == site).order_by(AiFaq.sort_order, AiFaq.id).all()
        return [
            {
                "id": f.id,
                "site": f.site,
...
def update_faq(faq_id: int, req: FaqRequest, request: Request, db: Session = Depends(get_db)):
    ...
    # Keep the existing scope for older clients that do not send site.
