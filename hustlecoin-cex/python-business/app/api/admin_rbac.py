...
def list_roles(request: Request, db: Session = Depends(get_db)):
    ...
    # A pre-RBAC database may reach this endpoint before the startup repair
    # has run (for example during a rolling deploy). Let a super admin repair
    # it on first view so the page never remains stuck at an empty catalog.
    if not rows and getattr(request.state, "role", None) == "SUPER_ADMIN":
        ensure_rbac_seed(db)
        rows = (
            db.query(Role, func.count(RolePermission.id).label("perm_count"))
            .outerjoin(RolePermission, Role.id == RolePermission.role_id)
            .group_by(Role.id)
            .order_by(Role.id)
            .all()
...
def ensure_rbac_seed(db: Session) -> dict[str, int]:
    """Idempotently create built-in RBAC data and migrate legacy user roles."""
    ...
    # Migrate the legacy users.role enum into normalized assignments.  Older
    # installations populated users.role but never populated the RBAC tables.
    role_by_code = {
        role.role_code: role
        for role in db.query(Role).filter(Role.role_code.in_([
            "super_admin", "system_admin", "observer"
        ])).all()
    }
    legacy_map = {
    ...
    for user in db.query(User).all():
        legacy_value = getattr(user.role, "value", user.role)
        role = role_by_code.get(legacy_map.get(str(legacy_value)))
        if role is None:
            continue
        exists = (
            db.query(UserRoleAssignment)
...
def seed_rbac(request: Request, db: Session = Depends(get_db)):
    ...
    result = ensure_rbac_seed(db)
    return {"message": "RBAC seed completed", **result}
