...
def _scoped_uid_snapshot(raw_payload, bound_sub_ids, now_ts):
    """Return UID telemetry only for the authenticated user's sub-accounts."""
    ids = {str(value) for value in (bound_sub_ids or ())}
    if not ids:
        return 0, 0, False
    try:
    ...
    if not isinstance(payload, dict):
        payload = {}
    rows = [value for key, value in payload.items()
            if str(key) in ids and isinstance(value, dict)]
    if not rows:
        return 0, 180000, True
    current = max(rows, key=lambda value: int(value.get("used_uid_weight_1m", 0) or 0))
    limit = int(current.get("uid_limit", 180000) or 180000)
    observed_at = float(current.get("uid_weight_time", 0) or 0)
    used = int(current.get("used_uid_weight_1m", 0) or 0)
    if observed_at <= 0 or now_ts - observed_at > 60:
        used = 0
...
def engine_health(request: Request, db: Session = Depends(get_db)):
    ...
    # IP-wide used weight is safe to share, but Binance UID weight is tied to
    # each bound sub-account. Never read the legacy process-wide max snapshot:
    # it belongs to whichever user the worker happened to service last.
    used_weight, weight_limit, weight_age = 0, 6000, None
    uid_used, uid_limit = 0, 0
    uid_scope_available = False
    throttle_rate = 0.0
    bound_sub_ids = {
        str(row[0]) for row in db.query(SubAccount.id).filter(
            SubAccount.user_id == user_id,
            SubAccount.is_enabled == True,
        ).all()
    }
    uid_scope_available = bool(bound_sub_ids)
    try:
        r = _redis()
        ...
        # `by_account` is shared, so select only IDs owned by this user.
        uid_used, uid_limit, uid_scope_available = _scoped_uid_snapshot(
            r.get("engine:uid_weight:by_account"),
            bound_sub_ids,
            datetime.now(timezone.utc).timestamp(),
        )
        UID_PER_BORROW = 1500
        pushed_raw = r.get(_user_redis_key(user_id, "pushed_symbols")) if uid_scope_available else None
        pushed_count = len(json.loads(pushed_raw)) if pushed_raw else 0
        uid_headroom = max(0, uid_limit - uid_used)
        per_acct_ceiling = uid_headroom / UID_PER_BORROW / 60 if uid_scope_available else 0.0
        ...
        # A telemetry outage must not reintroduce a global UID value.
        if not uid_scope_available:
            uid_used, uid_limit, throttle_rate = 0, 0, 0.0
    ...
    return HealthResponse(
        ...
        uid_limit=uid_limit,
        uid_scope_available=uid_scope_available,
