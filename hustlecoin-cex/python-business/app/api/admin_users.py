...
@router.get("/users/{user_id}/sub-accounts/{sa_id}/balance")
async def get_admin_sub_account_balance(user_id: int, sa_id: int, request: Request, db: Session = Depends(get_db)):
    require_admin(request)
    sa = db.query(SubAccount).filter(SubAccount.id == sa_id, SubAccount.user_id == user_id).first()
    if not sa:
        raise HTTPException(status_code=404, detail="Sub-account not found")
    ...
    try:
        async with BinanceTradingClient(sa.api_key, sa.api_secret) as client:
...
@router.get("/users/{user_id}/master-account/permissions")
async def get_admin_master_permissions(user_id: int, request: Request, db: Session = Depends(get_db)):
    require_admin(request)
