...
from app.api.admin_rbac import router as admin_rbac_router, ensure_rbac_seed
...
async def lifespan(app: FastAPI):
    ...
    # Normalize legacy users.role values into the RBAC catalog before the
    # admin UI queries it. This is idempotent and repairs installations where
    # users already had roles but roles/permissions tables were never seeded.
    db = None
    try:
        db = SessionLocal()
        rbac_result = ensure_rbac_seed(db)
        logger.info("RBAC catalog ensured: %s", rbac_result)
    except Exception as e:
        logger.warning("RBAC startup migration failed (non-fatal): %s", e)
        if db is not None:
            db.rollback()
    finally:
        if db is not None:
