    ...
    def _ensure_config(self):
        ...
        try:
            ...
            try:
                ...
                # Never route an unscoped worker to the first/admin user.
                # A recipient must be explicitly bound to this worker user.
                u = db.query(User).filter(User.id == self.user_id).first() if self.user_id is not None else None
                self._recipient_open_id = (getattr(u, "feishu_open_id", None) if u else None)
            except Exception as e:
