...
用途:coin 引擎 ①b-2 OKX 原生借币分支。key=joycar002 专用子账户(engine 机 .okx_borrow.env,与 dualperp/币安子账户全隔离)。
引擎版:每方法接受外部共享 httpx.AsyncClient(与主循环共享,事件循环安全)。
"""
import base64
import hashlib
import hmac
...
class OkxBorrow:
    ...
    async def ticker(self, cli, base: str) -> float:
        """base-USDT 现货最新价(qty 换算/穿价参考)。失败→0。"""
        r = (await cli.get(BASE + f"/api/v5/market/ticker?instId={base}-USDT")).json()
        if str(r.get("code")) == "0" and r.get("data"):
            return float(r["data"][0].get("last") or 0)
        return 0.0

    async def spot_lot(self, cli, base: str) -> tuple[float, float]:
        """现货 (minSz, lotSz)。失败→(0,0.0001)。"""
        r = (await cli.get(BASE + f"/api/v5/public/instruments?instType=SPOT&instId={base}-USDT")).json()
        if str(r.get("code")) == "0" and r.get("data"):
            d = r["data"][0]
            return float(d.get("minSz") or 0), float(d.get("lotSz") or 0.0001)
        return 0.0, 0.0001
