...
export interface AdminFundsAccount {
  ...
  /** Total margin debt valued in USDT, including non-USDT borrowed coins. */
  margin_borrowed_usdt: number
...
export interface AdminFundsMaster {
  account_name?: string | null
  snapshot_at_ms?: number | null
  snapshot_stale?: boolean
  spot_usdt_free: number
  spot_usdt_locked: number
  margin_net_usdt: number
  margin_usdt_free: number
  margin_usdt_borrowed: number
  /** Total margin debt valued in USDT, including non-USDT borrowed coins. */
...
export interface AdminFundsUser {
  ...
  username: string
  data_stale: boolean
  data_source?: 'redis' | 'snapshot' | 'none'
...
export interface AdminFundsOverview {
  ...
  totals: {
    ...
    master_equity: number
    deployed_notional: number
    /** Aggregate open-position notional. Newer backends expose this explicit alias. */
    total_position_notional?: number
    /** Realized PnL across the selected reporting window (usually today). */
    total_pnl?: number
    today_pnl?: number
    /** Number of positions closed in the selected reporting window. */
    total_closed?: number
    today_closed?: number
    users_with_data: number
    users_realtime?: number
    users_with_snapshot?: number
    users_total: number
  }
}
...
export interface NotificationTemplate {
  ...
  sound_key: string
  sound_persona_key?: string | null
  persona_key?: string | null
  sound_persona?: NotificationSoundPersona | null
  category_persona_default?: string
  is_enabled: boolean
}

export interface NotificationSoundPersona {
  id: number
  skey: string
  key: string
  name: string
...
export interface NotificationMaintenance {
  id: number
  enabled: boolean
  active: boolean
  stop_strategy: boolean
  block_trading: boolean
  block_login: boolean
  title: string
  content: string
  until_at: string | null
  updated_by: string | null
  status: 'MAINTENANCE' | 'NORMAL' | string
}

export async function getNotificationMaintenance() {
  const { data } = await client.get('/api/admin/notifications/maintenance')
  return data as NotificationMaintenance
}

export async function updateNotificationMaintenance(body: Partial<NotificationMaintenance>) {
  const { data } = await client.put('/api/admin/notifications/maintenance', body)
  return data as { saved: boolean; state: NotificationMaintenance; [key: string]: unknown }
}

export async function allStopNotificationMaintenance(body: Partial<NotificationMaintenance> = {}) {
  const { data } = await client.post('/api/admin/notifications/maintenance/all-stop', body, { __noRetry: true })
  return data as { saved: boolean; state: NotificationMaintenance; stop_commands?: unknown; [key: string]: unknown }
}

export async function listNotificationPersonas(includeDisabled = false) {
  const { data } = await client.get('/api/admin/notifications/personas', {
    params: includeDisabled ? { include_disabled: true } : undefined,
  })
  return data as NotificationSoundPersona[]
}

export async function getNotificationPersonaDefaults() {
  const { data } = await client.get('/api/admin/notifications/persona-defaults')
  return data as { mapping: Record<string, string>; categories: string[] }
}

export async function createNotificationPersona(body: Partial<NotificationSoundPersona> & { skey?: string }) {
  const { data } = await client.post('/api/admin/notifications/personas', body)
  return data as NotificationSoundPersona
}

export async function updateNotificationPersona(id: number, body: Partial<NotificationSoundPersona>) {
  const { data } = await client.put(`/api/admin/notifications/personas/${id}`, body)
  return data as NotificationSoundPersona
}

export async function deleteNotificationPersona(id: number) {
  const { data } = await client.delete(`/api/admin/notifications/personas/${id}`)
  return data as { deleted: boolean; id: number; disabled?: boolean }
}

export async function testNotificationPersona(id: number, text?: string) {
  const { data } = await client.post(`/api/admin/notifications/personas/${id}/test`, null, {
    params: text ? { text } : undefined,
  })
  return data as { persona: NotificationSoundPersona; text: string; server_tts: boolean; tts_url?: string }
}

/** Request a cached server-side Chinese female neural-voice clip. */
export async function getNotificationTts(params: {
  text: string
  persona?: string | null
  voice?: string | null
  ...
  const { data } = await client.get('/api/admin/notifications/tts', {
    params,
    responseType: 'blob',
    timeout: 30000,
    __noRetry: true,
...
export async function pregenNotificationTts() {
  const { data } = await client.post('/api/admin/notifications/tts/pregen', null, {
    timeout: 120000,
    __noRetry: true,
  })
  return data as { templates: number; jobs: number; generated: number; cached: number; failed: string[] }
}
...
export async function getSystemInfo() {
  // These are read-only status calls. A stalled remote service must not leave
  // the whole version tab in an eternal loading state or trigger duplicate
  // retries while the operator is already looking at the page.
  const { data } = await client.get('/api/admin/system/info', { timeout: 10000, __noRetry: true })
...
export async function getGitHistory() {
  const { data } = await client.get('/api/admin/system/git-history', { timeout: 10000, __noRetry: true })
...
export async function getServiceVersions() {
  const { data } = await client.get('/api/admin/system/versions', { timeout: 10000, __noRetry: true })
  return data as { services: ServiceVersion[] }
}

export interface ServerMonitorSnapshot { host: string; reachable: boolean; healthy: boolean; status: string; cpu_load_1m: number; memory_percent: number; disk_percent: number; services: Record<string, string>; latency_ms: number }
export interface MonitorSubscription { id: number | null; user_id: number; username: string; email_address?: string | null; feishu_open_id?: string | null; target: string; enabled: boolean; feishu: boolean; email: boolean }
export async function getServerMonitoring() { const { data } = await client.get('/api/admin/system/monitoring', { timeout: 12000, __noRetry: true }); return data as { checked_at: string; servers: Record<string, ServerMonitorSnapshot> } }
export async function getMonitorSubscriptions() { const { data } = await client.get('/api/admin/system/monitoring/subscriptions'); return data as MonitorSubscription[] }
export async function updateMonitorSubscription(userId: number, body: { target: string; enabled: boolean; feishu: boolean; email: boolean }) { const { data } = await client.put(`/api/admin/system/monitoring/subscriptions/${userId}`, body); return data as { saved: boolean } }
...
export async function listAiFaq(site = 'default') {
  const { data } = await client.get('/api/admin/ai/faq', { params: { site } })
  return data as AiFaqItem[]
}

export async function createAiFaq(body: { question: string; answer: string; category: string; sort_order?: number }, site = 'default') {
  const { data } = await client.post('/api/admin/ai/faq', body, { params: { site } })
  return data
}

export async function updateAiFaq(id: number, body: Partial<AiFaqItem>, site = 'default') {
  const { data } = await client.put(`/api/admin/ai/faq/${id}`, body, { params: { site } })
  return data
}

export async function deleteAiFaq(id: number, site = 'default') {
  await client.delete(`/api/admin/ai/faq/${id}`, { params: { site } })
}
...
export interface GlobalRulesData {
  ...
  early_borrow_auto_push_enabled?: boolean
  ...
  inventory_probe_rate_per_sec?: string | null
  borrow_via_otoco?: boolean
  borrow_mode?: string | null
  otoco_legs?: number
  multi_max_accounts_per_symbol?: number
  hedge_via_master?: boolean
  // Strategy-economics and market-monitoring fields returned by the backend.
  net_gate_mode?: string | null
  spot_order_mode?: string | null
  hedge_auto_converge?: boolean
...
export interface ServerMonitorItem {
  host: string
  name: string
  reachable: boolean
  service: string
...
export async function getServerMonitor() {
  const { data } = await client.get('/api/admin/system/server-monitor', { timeout: 12000, __noRetry: true })
  return data as { servers: ServerMonitorItem[] }
}
...
export interface DelistingMonitorConfig {
  id: number
  enabled: boolean
  interval_sec: number
  ...
  telegram_session_configured: boolean
  telegram_chat: string | null
  last_scan_at: string | null
  last_error: string | null
}

export async function getDelistingConfig() {
  const { data } = await client.get('/api/admin/market/delisting-config')
  return data as DelistingMonitorConfig
}

export async function updateDelistingConfig(body: Record<string, unknown>) {
  const { data } = await client.put('/api/admin/market/delisting-config', body)
  return data as DelistingMonitorConfig
}

export async function triggerDelistingScan() {
  const { data } = await client.post('/api/admin/market/delisting-scan')
  return data as { found: string[]; count: number }
}
