...
const SITE = 'coinadmin'
const TOKEN_KEY = 'admin_jwt_token'
const SESSION_KEY = 'ai_chat_session_coinadmin'
    ...
    <>
      {!open && (
        <button onClick={() => setOpen(true)}
          className="fixed bottom-4 right-4 z-50 flex h-12 w-12 items-center justify-center rounded-full shadow-lg transition-transform hover:scale-110 md:bottom-6 md:right-6 md:h-14 md:w-14"
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-background md:inset-auto md:bottom-6 md:right-6 md:h-[520px] md:w-[400px] md:rounded-xl md:shadow-2xl md:border">
          ...
          <div className="flex items-center justify-between rounded-t-xl px-4 py-3 text-white"
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium text-sm">{view === 'chat' ? 'CoinAdmin 管理助手' : '对话记录'}</span>
            </div>
                ...
                {messages.length === 0 && (
                  <div className="flex items-center justify-center h-full">
                    <p className="text-sm text-muted-foreground text-center px-4">
                      你好！我是 CoinAdmin 管理助手，系统管理和运维问题都可以问我。
                    </p>
                  </div>
                )}
                {messages.map((m, i) => (
