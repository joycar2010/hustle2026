...
export function FundsPage() {
  ...
  const t = data?.totals
  // Position/PnL totals are supplied by the funds endpoint. Keep aliases for
  // older deployments so the card remains useful while the backend rolls out
  // the explicit field names.
  const positionNotional = t?.total_position_notional ?? t?.deployed_notional
  const totalPnl = t?.total_pnl ?? t?.today_pnl
  const totalClosed = t?.total_closed ?? t?.today_closed
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-9 gap-2">
        <StatCard label="主账户净值" value={t ? `${formatNumber(t.master_equity)} U` : '-'} hint="已计入平台净值" />
        <StatCard label="全平台净值" value={t ? `${formatNumber(t.equity)} U` : '-'} accent="text-primary" hint={t ? `${t.users_with_data}/${t.users_total} 用户有数据${t.users_realtime != null && t.users_realtime < t.users_with_data ? ` · 实时 ${t.users_realtime}` : ''}` : undefined} />
        ...
        <StatCard label="总持仓总额" value={positionNotional != null ? `${formatNumber(positionNotional)} U` : '-'} hint="当前未结束仓位名义额" />
        <StatCard label="总 PNL" value={totalPnl != null ? `${totalPnl >= 0 ? '+' : ''}${formatNumber(totalPnl)} U` : '-'} accent={pnlColor(totalPnl ?? null)} hint={t ? `累计已平仓实现盈亏 · 今日 ${formatNumber(t.today_pnl ?? 0)} U` : undefined} />
        <StatCard label="总平仓" value={totalClosed != null ? `${totalClosed} 笔` : '-'} hint={t ? `累计完成平仓 · 今日 ${t.today_closed ?? 0} 笔` : undefined} />
          ...
          <table className="w-full md:min-w-[860px] text-[11px] border-collapse">
            <thead>
              <tr className="bg-[#0d0d14] text-muted-foreground border-b border-border">
                <th className="px-3 py-2 text-right font-medium">主账户</th>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={10} className="px-3 py-8 text-center text-muted-foreground">加载中...</td></tr>
              ) : !data || data.users.length === 0 ? (
                <tr><td colSpan={10} className="px-3 py-8 text-center text-muted-foreground">暂无用户</td></tr>
                ...
                return (
                  ...
                  >
                    <td className="px-3 py-1.5 text-right tabular-nums font-mono text-primary">{u.master ? formatNumber(u.master.equity) : '—'}</td>
                    <td className="px-3 py-1.5 font-medium">
                      <span className="text-muted-foreground mr-1">{isExp ? '▾' : '▸'}</span>
                      {u.username}
                      {u.data_stale && <span title={u.data_source === 'snapshot' && u.snapshot_at ? `最后快照 ${u.snapshot_at}` : '引擎尚未推送余额快照'} className="ml-1.5 px-1 py-0.5 rounded bg-amber-400/15 text-amber-400 text-[9px] align-middle">{u.data_source === 'snapshot' ? '历史快照' : '暂无快照'}</span>}
                    </td>
                  ...
                  {isExp && (u.accounts.length > 0 || u.master) && (
                          ...
                          <tbody>
                            {u.master && (
                              <tr className="border-t border-primary/30 bg-primary/5">
                                <td className="pr-3 py-0.5 text-primary font-semibold">主账户{u.master.snapshot_stale ? ' · 陈旧' : ''}</td>
                                <td className="pr-3 text-right tabular-nums text-primary">{formatNumber(u.master.equity)}</td>
                                <td className="pr-3 text-right tabular-nums">{formatNumber(u.master.spot_usdt_free)}</td>
                                <td className="pr-3 text-right tabular-nums">{formatNumber(u.master.margin_net_usdt)}</td>
                                <td className="pr-3 text-right tabular-nums">{formatNumber(u.master.margin_usdt_borrowed)}</td>
                                <td className="pr-3 text-right tabular-nums">{formatNumber(u.master.futures_total)}</td>
                                <td className={cn('pr-3 text-right tabular-nums', pnlColor(u.master.futures_unrealized_pnl))}>{formatNumber(u.master.futures_unrealized_pnl)}</td>
                                <td className={cn('pr-3 text-right tabular-nums', marginColor(u.master.margin_level < 100 ? u.master.margin_level : null))}>{u.master.margin_level >= 100 ? '∞' : u.master.margin_level.toFixed(2)}</td>
                                <td className="text-right tabular-nums">{u.master.bnb_free.toFixed(4)}</td>
                              </tr>
                            )}
                            {u.accounts.map((a) => (
                              <tr key={a.account_id} className="border-t border-border/20">
                                ...
                                <td className="pr-3 text-right tabular-nums">{formatNumber(a.margin_borrowed_usdt)}</td>
