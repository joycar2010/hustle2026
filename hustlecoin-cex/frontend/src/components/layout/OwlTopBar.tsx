...
function playSynthSound(key: string) {
  if (key === 'none' || localStorage.getItem('coin.sound.enabled') === '0') return
  try {
...
// Browser TTS fallback: use a Chinese female voice for actionable notifications.
function speakNotification(title: string, content: string, sound: string) {
  if (localStorage.getItem('coin.sound.enabled') === '0' || !('speechSynthesis' in window)) return
  try {
...
function NotificationMarquee() {
  ...
  // Keep notification playback in sync with the top-bar speaker switch.
  const [soundEnabled, setSoundEnabled] = useState(() => {
    try { return localStorage.getItem('coin.sound.enabled') !== '0' } catch { return true }
  })

  useEffect(() => {
    const soundToggle = (e: Event) => {
      const enabled = (e as CustomEvent<boolean>).detail
      if (typeof enabled === 'boolean') setSoundEnabled(enabled)
    }
    window.addEventListener('coin:sound-toggle', soundToggle)
    const handler = (e: Event) => {
      ...
      if (soundEnabled) {
        ...
        speakNotification(item.title, item.content, item.sound)
      }
    }
    window.addEventListener('ws:notification', handler)
    return () => {
      window.removeEventListener('coin:sound-toggle', soundToggle)
...
export function OwlTopBar() {
  ...
  const [soundEnabled, setSoundEnabled] = useState(() => localStorage.getItem('coin.sound.enabled') !== '0')
  const toggleSound = useCallback(() => {
    setSoundEnabled((enabled) => {
      const next = !enabled
      localStorage.setItem('coin.sound.enabled', next ? '1' : '0')
      window.dispatchEvent(new CustomEvent('coin:sound-toggle', { detail: next }))
      return next
    })
  }, [])
    ...
    <>
      <header className="flex h-10 items-center border-b border-border bg-[#0d0d12] px-3 gap-2 shrink-0 select-none">
        ...
        <div className="hidden md:flex items-center gap-1 ml-2 shrink-0">
          <button type="button" onClick={toggleSound} title={soundEnabled ? '关闭提示音' : '开启提示音'} aria-label={soundEnabled ? '关闭提示音' : '开启提示音'} className="p-1 rounded text-muted-foreground hover:text-foreground hover:bg-accent transition-colors">
        ...
        <button type="button" onClick={toggleSound} title={soundEnabled ? '关闭提示音' : '开启提示音'} aria-label={soundEnabled ? '关闭提示音' : '开启提示音'} className="md:hidden p-1.5 rounded text-muted-foreground hover:text-foreground hover:bg-accent">
