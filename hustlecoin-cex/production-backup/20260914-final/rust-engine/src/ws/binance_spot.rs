...
pub async fn run(
    ...
    loop {
        if symbols.is_empty() {
            warn!("Spot WS has no eligible symbols; waiting for engine:universe");
...
async fn connect_and_stream(
    ...
    // The 24h ticker stream emits once per second and includes the executable
    // best bid/ask fields (`b`/`a`). `bookTicker` only emits on book changes,
    // which made quiet but tradable symbols look stale under the hard freshness
    // guard. The local receive timestamp remains the safety authority.
    // Binance Combined Stream names are lowercase. The socket can handshake
    // with uppercase names but then emits no ticker frames.
                        ...
                        if let tokio_tungstenite::tungstenite::Message::Text(text) = msg {
                            if let Ok(combined) = serde_json::from_str::<BinanceCombinedStream>(&text) {
                                ...
                                let (Some(bid), Some(ask)) =
                                    (combined.data.bid_price, combined.data.ask_price)
                                else {
                                    continue;
                                };
                                let spot = TickerData {
                    ...
                    let stale = chunk
                        ...
                        .filter(|s| {
                            ...
                            let last = mon_tickers
                                .get(key.as_str())
                                .map(|t| if t.0.recv_ts > 0 { t.0.recv_ts } else { t.0.ts })
                                .unwrap_or(0);
    ...
    // The market pusher can replace an execution symbol with an exit-only
    // symbol without disconnecting Binance. Poll the merged Redis universe so
    // subscriptions converge within a few seconds rather than waiting for a
    // remote socket restart. Redis/JSON failures are ignored and retain the
    // current feed until a valid snapshot is available.
    {
    ...
    while let Some(result) = tasks.join_next().await {
        match result {
            ...
            Err(error) => {
                tasks.abort_all();
                while tasks.join_next().await.is_some() {}
                return Err(format!("Spot WS task join error: {}", error).into());
            }
        }
    }
    Ok(())
