...
AGENTS_STATUS_PATH = ROOT / "agents" / "out" / "agent_status.json"
_SLOW_LOOP_ALIAS = {"review": "desk"}  # review work surfaces under the desk card
_SLOW_LOOP_MAX_AGE_SEC = 1200          # timer cadence 10min + margin


def _slow_loop_overlay(snapshot: dict[str, Any]) -> dict[str, Any]:
    """Overlay REAL per-role slow-loop status (agents/run.py) onto the base
    snapshot.  Read-only and failure-silent: missing/stale file changes
    nothing."""
    try:
        if not AGENTS_STATUS_PATH.exists():
            return snapshot
        if time.time() - AGENTS_STATUS_PATH.stat().st_mtime > _SLOW_LOOP_MAX_AGE_SEC:
            return snapshot
        payload = json.loads(AGENTS_STATUS_PATH.read_text(encoding="utf-8"))
        for role, info in (payload.get("roles") or {}).items():
            if not isinstance(info, dict) or info.get("kind") != "agent":
                continue
            target = _SLOW_LOOP_ALIAS.get(role, role)
            entry = snapshot.get(target)
            if entry is None:
                continue
            slow = entry.setdefault("slow_loop", {})
            slow[role] = {k: info.get(k) for k in (
                "status", "workflow_stage", "duration_ms", "heartbeat_ts",
                "n_reviews", "n_degraded", "whale_trades", "detail") if k in info}
            stage = info.get("workflow_stage")
            if stage and stage != "-" and role == target:
                entry["workflow_stage"] = f"slow-loop:{stage}"
            if info.get("status") == "error" and role == target:
                entry["status"] = "error"
                entry["detail"] = info.get("detail") or entry.get("detail")
...
def _agent_status_snapshot() -> dict[str, Any]:
        ...
        if heartbeat_path.exists() and time.time() - heartbeat_path.stat().st_mtime < 20:
            ...
            if isinstance(payload, dict):
                ...
                if instance == expected and pid_alive and isinstance(roles, dict) and all(role in roles for role in OPENCLAW_AGENT_IDS):
                    ...
                    return _slow_loop_overlay(normalized)
    except (OSError, ValueError, TypeError):
        pass
    ...
    return _slow_loop_overlay(result)
...
def _build_sports_data_source():
    """Bookmaker-consensus probabilities when ODDS_API_KEY is set; None keeps
    the market-implied fallback (edge stays ~0, gate stays quiet)."""
    api_key = os.environ.get("ODDS_API_KEY", "").strip()
    switch = os.environ.get("SPORTS_ODDS_SOURCE", "the-odds-api").strip().lower()
    if not api_key or switch in ("off", "0", "false", "none"):
        return None
    try:
        from bot.feeds.odds_consensus import OddsConsensusSource
        return OddsConsensusSource(api_key=api_key)
    except Exception:
        return None


_sports_service = SportsPaperService(cfg, data_source=_build_sports_data_source())
...
def realized_pnl_today() -> dict:
    ...
    """
    return realized_pnl_summary_today(_state.get("bot_mode") != "live")


_paper_open_cache: dict[str, Any] = {"ts": 0.0, "n": 0, "usd": 0.0}


def _paper_open_stats() -> tuple[int, float]:
    """Open (unresolved) paper entries: count and notional, 2s cached."""
    now = time.time()
    if now - _paper_open_cache["ts"] > 2.0:
        n, usd = 0, 0.0
        try:
            with db() as c:
                row = c.execute(
                    "SELECT COUNT(*), COALESCE(SUM((o.size-COALESCE(x.exit_size,0))*o.price),0) "
                    "FROM orders o LEFT JOIN (SELECT condition_id, token_id, dry_run, "
                    "SUM(size) AS exit_size FROM orders WHERE status='exit_dry_run' "
                    "GROUP BY condition_id, token_id, dry_run) x "
                    "ON x.condition_id=o.condition_id AND x.token_id=o.token_id AND x.dry_run=o.dry_run "
                    "WHERE o.dry_run=1 AND o.status='dry_run' AND o.side IN ('UP','DOWN') "
                    "AND NOT EXISTS (SELECT 1 FROM resolutions r WHERE r.condition_id=o.condition_id) "
                    "AND o.size-COALESCE(x.exit_size,0) > 0.02"
                ).fetchone()
            if row:
                n, usd = int(row[0] or 0), float(row[1] or 0.0)
        except Exception:
            pass
        _paper_open_cache.update(ts=now, n=n, usd=round(usd, 4))
    return _paper_open_cache["n"], _paper_open_cache["usd"]

_ledger_totals_cache: dict[str, Any] = {"ts": 0.0, "decisions": 0, "orders": 0}


def _ledger_totals() -> dict[str, int]:
    """Real ledger totals (MAX id) for the bottom bar, 2s cached."""
    now = time.time()
    if now - _ledger_totals_cache["ts"] > 2.0:
        try:
            with db() as c:
                d = c.execute("SELECT COALESCE(MAX(id),0) FROM decisions").fetchone()[0]
                o = c.execute("SELECT COALESCE(MAX(id),0) FROM orders").fetchone()[0]
            _ledger_totals_cache.update(ts=now, decisions=int(d), orders=int(o))
        except Exception:
            _ledger_totals_cache["ts"] = now
    return {"decisions": _ledger_totals_cache["decisions"],
            "orders": _ledger_totals_cache["orders"]}

_feeds_status_cache: dict[str, Any] = {"ts": 0.0, "value": {}}


def _feeds_status() -> dict[str, Any]:
    """External-feed health for the UI, 3s cached."""
    now = time.monotonic()
    if now - _feeds_status_cache["ts"] < 3.0:
        return _feeds_status_cache["value"]
    kalshi = dict(_state.get("kalshi_weather") or {})
    try:
        from bot.feeds.kalshi_weather import LAST_STATS
        kalshi.setdefault("scanned", LAST_STATS.get("scanned", 0))
        kalshi.setdefault("quoted", LAST_STATS.get("quoted", 0))
    except Exception:
        pass
    odds: dict[str, Any] = {
        "source": type(getattr(_sports_service, "data_source", None)).__name__,
    }
    try:
        budget_path = ROOT / "agents" / "out" / "odds_budget.json"
        if budget_path.exists():
            odds["remaining"] = json.loads(budget_path.read_text()).get("remaining")
        matches_path = ROOT / "agents" / "out" / "odds_matches.jsonl"
        odds["matches"] = sum(1 for _ in matches_path.open()) if matches_path.exists() else 0
        src = getattr(_sports_service, "data_source", None)
        cache = getattr(src, "_cache", None)
        if isinstance(cache, dict):
            odds["events_cached"] = sum(len(v[1]) for v in cache.values())
    except Exception:
        pass
    value = {"kalshi_weather": kalshi, "odds": odds}
    _feeds_status_cache.update(ts=now, value=value)
    return value



_paper_positions_cache: dict[str, Any] = {"ts": 0.0, "rows": []}


def _paper_positions() -> list[dict[str, Any]]:
    """Unresolved dry-run entries (the rows behind the paper open value)."""
    now = time.time()
    if now - _paper_positions_cache["ts"] > 2.0:
        rows: list[dict[str, Any]] = []
        try:
            with db() as c:
                for r in c.execute(
                    "SELECT o.ts, o.market_slug, COALESCE(o.asset,'BTC') AS asset, o.side, "
                    "o.size-COALESCE(x.exit_size,0) AS size, o.price FROM orders o "
                    "LEFT JOIN (SELECT condition_id, token_id, dry_run, SUM(size) AS exit_size "
                    "FROM orders WHERE status='exit_dry_run' GROUP BY condition_id, token_id, dry_run) x "
                    "ON x.condition_id=o.condition_id AND x.token_id=o.token_id AND x.dry_run=o.dry_run "
                    "WHERE o.dry_run=1 AND o.status='dry_run' AND o.side IN ('UP','DOWN') "
                    "AND NOT EXISTS (SELECT 1 FROM resolutions r WHERE r.condition_id=o.condition_id) "
                    "AND o.size-COALESCE(x.exit_size,0) > 0.02 ORDER BY o.ts DESC LIMIT 50"
                ):
                    ts, slug, asset, side, size, price = r
                    rows.append({
                        "ts": ts, "market_slug": slug, "asset": asset,
                        "side": side, "size": size, "price": price,
                        "notional_usd": round((size or 0) * (price or 0), 4),
                    })
        except Exception:
            rows = _paper_positions_cache["rows"]
        _paper_positions_cache.update(ts=now, rows=rows)
    return _paper_positions_cache["rows"]


_asset_pnl_cache: dict[str, Any] = {"ts": 0.0, "key": None, "value": {}}


def _asset_lane_pnl(period_start: float) -> dict[str, dict[str, Any]]:
    """Settled ENTRY PnL per (lane, asset) since period_start.

    Entry-order scope (exits excluded), matching backtest attribution; the
    headline pnl stays the canonical store number -- this powers the per-
    asset cards only."""
    now = time.time()
    if now - _asset_pnl_cache["ts"] > 5.0 or _asset_pnl_cache["key"] != int(period_start):
        out: dict[str, dict[str, Any]] = {"paper": {}, "live": {}}
        try:
            with db() as c:
                for r in c.execute(
                    "SELECT COALESCE(o.asset,'BTC'), o.dry_run, o.side, o.token_id, "
                    "o.size, o.price, o.filled_size, o.status, r.winning_token "
                    "FROM orders o JOIN resolutions r ON r.condition_id=o.condition_id "
                    "WHERE r.resolved_ts >= ? AND o.side IN ('UP','DOWN') "
                    "AND o.status IN ('filled','matched','dry_run')",
                    (period_start,),
                ):
                    asset, dry, _side, tok, size, price, filled, status, win_tok = r
                    if not price or not (0 < price < 1):
                        continue
                    if status in ("filled", "matched"):
                        stake = filled or (size or 0) * price  # USD notional
                        shares = stake / price
                    else:
                        shares = size or 0.0
                        stake = shares * price
                    if shares <= 0:
                        continue
                    lane = "paper" if dry else "live"
                    a = out[lane].setdefault(asset, {"realized_usd": 0.0, "wins": 0, "losses": 0})
                    if tok == win_tok:
                        a["realized_usd"] += shares * (1 - price)
                        a["wins"] += 1
                    else:
                        a["realized_usd"] -= stake
                        a["losses"] += 1
            for lane in out.values():
                for a in lane.values():
                    a["realized_usd"] = round(a["realized_usd"], 4)
        except Exception:
            out = _asset_pnl_cache["value"]
        _asset_pnl_cache.update(ts=now, key=int(period_start), value=out)
    return _asset_pnl_cache["value"]


def _mode_split() -> dict[str, Any]:
    """Paper and live KPI families side by side so the UI can label lanes.

    balance_pusd stays chain-truth and mode-free; this block covers realized
    PnL (today), win/loss, pending and open exposure per lane."""
    paper = dict(realized_pnl_summary_today(True))
    live = dict(realized_pnl_summary_today(False))
    n, usd = _paper_open_stats()
    paper.update(open_count=n, open_value_usd=usd)
    live_positions = _filter_active_positions(_state.get("positions") or [])
    live_value = 0.0
    for item in live_positions:
        try:
            live_value += float(item.get("size") or 0) * float(item.get("curPrice") or 0)
        except (TypeError, ValueError):
            continue
    live.update(open_count=len(live_positions), open_value_usd=round(live_value, 4))
    paper["capital_base_usd"] = round(float(getattr(cfg, "paper_equity_usd", 100.0)), 2)
    live["capital_base_usd"] = round(float(_state.get("balance_pusd") or 0.0) + live_value, 4)
    # by_asset now arrives inside the store summaries themselves (lot
    # accounting, patch #20); the entry-attribution helper is retired from
    # this path so cards and KPIs are views of one computation.
    paper.setdefault("by_asset", {})
    live.setdefault("by_asset", {})
    return {"paper": paper, "live": live}



_slow_loop_cache: dict[str, Any] = {"key": None, "value": {}}


def _slow_loop_extras() -> dict[str, Any]:
    """Pending agent proposals + latest desk digest, mtime-cached."""
    props_path = ROOT / "agents" / "out" / "proposals.jsonl"
    dig_path = ROOT / "agents" / "out" / "digests.jsonl"
    try:
        key = (props_path.stat().st_mtime if props_path.exists() else 0,
               dig_path.stat().st_mtime if dig_path.exists() else 0)
    except OSError:
        key = (0, 0)
    if _slow_loop_cache["key"] == key:
        return _slow_loop_cache["value"]
    proposals, digest = [], None
    try:
        if props_path.exists():
            latest: dict[str, dict] = {}
            for line in props_path.read_text(encoding="utf-8").strip().splitlines()[-40:]:
                try:
                    p = json.loads(line)
                except ValueError:
                    continue
                if p.get("status") == "PENDING_HUMAN":
                    latest[str(p.get("key")) + ":" + str(p.get("proposed"))] = p
            proposals = sorted(latest.values(), key=lambda p: -float(p.get("ts") or 0))[:6]
        if dig_path.exists():
            tail = dig_path.read_text(encoding="utf-8").strip().splitlines()
            if tail:
                d = json.loads(tail[-1])
                digest = {k: d.get(k) for k in (
                    "period", "reviews", "total_pnl_usd", "win_rate", "consensus", "ts")}
    except (OSError, ValueError):
        pass
    value = {"proposals": proposals, "digest": digest}
    _slow_loop_cache.update(key=key, value=value)
    return value
...
async def poll_btc_signal_loop():
    while True:
        ...
        try:
            ...
            _btc_signal_ok["ts"] = time.time()
        except Exception as e:
            _state["btc_signal"] = None
            # Window-boundary candle lag is expected; surface only after
            # 30s of continuous failure (same policy as eth_signal).
            if time.time() - _btc_signal_ok["ts"] > 30.0:
                _state["errors"]["btc_signal"] = str(e)
        await asyncio.sleep(1.0)


_eth_signal_ok = {"ts": time.time()}
_btc_signal_ok = {"ts": time.time()}
...
async def poll_eth_signal_loop():
    while True:
        ...
        try:
            ...
            _state["eth_signal"] = None
            # Debounce transient feed hiccups: surface only after 30s of
            # continuous failure so one bad poll cannot red-flag the UI.
            if time.time() - _eth_signal_ok["ts"] > 30.0:
...
def _weather_feed_snapshot() -> list[dict[str, Any]]:
    ...
    for market in markets[:12]:
        try:
            ...
            _maybe_weather_paper_entry(market, signal)
...
async def poll_sports_loop():
    ...
    while True:
        try:
            ...
            _persist_sports_paper(snapshot)
...
async def poll_kalshi_weather_loop():
    """Kalshi daily high-temp brackets vs the Open-Meteo ensemble model.

    Reference prices only -- paper entries land in OUR ledger through the
    same hook the Polymarket weather branch uses; settlements are polled
    from Kalshi's public API into the local resolutions table."""
    if os.environ.get("KALSHI_WEATHER_ENABLED", "true").strip().lower() not in (
            "1", "true", "yes", "on"):
        return
    from bot.feeds.kalshi_weather import (
        fetch_kalshi_weather_markets,
        resolve_kalshi_settlements,
    )
    while True:
        try:
            markets = await asyncio.to_thread(fetch_kalshi_weather_markets)
            for market in markets:
                try:
                    forecast = await asyncio.to_thread(
                        fetch_ensemble_forecast, cfg, market.city_key, market.target_date)
                except Exception:
                    continue
                signal = build_signal(cfg, market, forecast)
                _maybe_weather_paper_entry(market, signal)
            settled = await asyncio.to_thread(resolve_kalshi_settlements)
            _state["kalshi_weather"] = {
                "markets": len(markets), "settled_new": settled,
                "ts": time.time(),
            }
            _state["errors"].pop("kalshi_weather", None)
        except Exception as exc:
            _state["errors"]["kalshi_weather"] = str(exc)
...
async def _startup():
    ...
    asyncio.create_task(poll_kalshi_weather_loop())
...
def _risk_state(pnl: dict) -> str:
    ...
    if _state.get("bot_mode") != "live":
        # Match the worker gate: paper capital + paper open exposure.
        equity = float(getattr(cfg, "paper_equity_usd", 100.0)) + _paper_open_stats()[1]
    else:
        balance = _state.get("balance_pusd")
        value = _state.get("value_usd") or 0.0
        equity = (balance if balance is not None else 0.0) + value
    if (
...
def _risk_snapshot(pnl: dict) -> dict[str, Any]:
    ...
    return {
        "state": risk_state,
        "position_count": len(positions) if _state.get("bot_mode") == "live" else (len(positions) or _paper_open_stats()[0]),
        "position_value_usd": round(position_value, 4) if _state.get("bot_mode") == "live" else round(position_value or _paper_open_stats()[1], 4),
...
_ws_frame: dict[str, Any] = {"text": "", "event": None}
_ws_producer_task: "asyncio.Task | None" = None


async def _ws_produce_loop() -> None:
    """Publish frames ON a fixed 1.0s monotonic grid.

    Compute happens between ticks; the event flip (which sets the spacing
    every client sees) lands exactly on the grid, so compute-time variance
    becomes bounded latency instead of cadence jitter."""
    next_tick = time.monotonic() + 0.25
    while True:
        try:
            payload = await asyncio.to_thread(state)
            text = json.dumps(payload, default=str)
        except Exception:
            text = _ws_frame["text"]  # keep pushing the last good frame
        now = time.monotonic()
        while next_tick <= now:  # overran the budget: skip missed grid points
            next_tick += 0.25
        await asyncio.sleep(next_tick - now)
        next_tick += 0.25
        prev_event = _ws_frame["event"]
        _ws_frame["text"] = text
        _ws_frame["event"] = asyncio.Event()
...
def _paper_entry_logged(condition_id: str) -> bool:
    try:
        return store.has_executed_order(condition_id, True)
    except Exception:
        return True  # fail closed: never double-log


def _persist_sports_paper(snapshot) -> None:
    """Copy the sports service's simulated orders into the real paper ledger
    so resolution/PnL/by-asset all treat them like any other dry-run entry."""
    try:
        get = lambda o, k, d=None: o.get(k, d) if isinstance(o, dict) else getattr(o, k, d)
        markets = {get(m, "market_key"): m for m in (get(snapshot, "markets", []) or [])}
        for o in (get(snapshot, "orders", []) or []):
            cid = str(get(o, "market_key") or "")
            token = str(get(o, "token_id") or "")
            price = float(get(o, "price") or 0)
            size = float(get(o, "size") or 0)
            slug = str(get(o, "slug") or "")
            if not cid or not token or not (0 < price < 1) or size <= 0:
                continue
            if _paper_entry_logged(cid):
                continue
            m = markets.get(cid)
            tokens = list(get(m, "clob_token_ids", []) or []) if m is not None else []
            side = "UP" if (tokens and str(tokens[0]) == token) else "DOWN"
            store.log_order(asset="SPORTS", market_slug=slug, condition_id=cid,
                            token_id=token, side=side, size=size, price=price,
                            order_id=None, status="dry_run", filled_size=0.0,
                            entry_rule="sports_paper", dry_run=True)
            store.log_decision(asset="SPORTS", market_slug=slug, condition_id=cid,
                               token_id=token, side=side, t_remaining=0.0,
                               ask_price=price, ask_size=None, action="BUY",
                               reason=f"sports paper {get(o, 'outcome', '')} "
                                      f"edge entry @ {price}", dry_run=True)
    except Exception:
        pass  # persistence must never break the sports poller


_WEATHER_PAPER_ENABLED = os.environ.get(
    "WEATHER_PAPER_TRADING", "true").strip().lower() in ("1", "true", "yes", "on")
_WEATHER_PAPER_MIN_EDGE = 0.10


def _maybe_weather_paper_entry(market, signal) -> None:
    """Book a $1 paper entry when the ensemble model disagrees with the
    market by >= 10 cents.  Runs inside the weather snapshot loop; dormant
    while no weather markets match."""
    if not _WEATHER_PAPER_ENABLED:
        return
    try:
        model_yes = float(getattr(signal, "model_yes_probability", None) or 0.0)
        yes_price = float(getattr(market, "yes_price", 0) or 0)
        no_price = float(getattr(market, "no_price", 0) or 0)
        cid = str(getattr(market, "condition_id", "") or "")
        slug = str(getattr(market, "slug", "") or "")
        if not cid or _paper_entry_logged(cid):
            return
        pick = None
        if 0.02 < yes_price < 0.95 and model_yes - yes_price >= _WEATHER_PAPER_MIN_EDGE:
            pick = ("UP", str(getattr(market, "yes_token", "")), yes_price, model_yes - yes_price)
        elif 0.02 < no_price < 0.95 and (1.0 - model_yes) - no_price >= _WEATHER_PAPER_MIN_EDGE:
            pick = ("DOWN", str(getattr(market, "no_token", "")), no_price, (1.0 - model_yes) - no_price)
        if pick is None:
            return
        side, token, price, edge = pick
        if not token:
            return
        size = round(1.0 / price, 4)  # $1 notional
        store.log_order(asset="WEATHER", market_slug=slug, condition_id=cid,
                        token_id=token, side=side, size=size, price=price,
                        order_id=None, status="dry_run", filled_size=0.0,
                        entry_rule="weather_paper", dry_run=True)
        store.log_decision(asset="WEATHER", market_slug=slug, condition_id=cid,
                           token_id=token, side=side, t_remaining=0.0,
                           ask_price=price, ask_size=None, action="BUY",
                           reason=f"weather ensemble edge {edge:+.2f} @ {price}",
                           dry_run=True)
    except Exception:
        pass


_TRADING_DAY_SHIFT = int(float(os.environ.get("TRADING_DAY_UTC_OFFSET_HOURS", "8")) * 3600)


_consensus_hist_cache: dict[str, Any] = {"key": None, "rows": []}


@app.get("/api/consensus/history")
def consensus_history(offset: int = 0, limit: int = 10):
    """Primary vs secondary model opinions per desk cycle, newest first."""
    offset = max(0, min(int(offset), 5000))
    limit = max(1, min(int(limit), 50))
    path = ROOT / "agents" / "out" / "digests.jsonl"
    try:
        key = path.stat().st_mtime
    except OSError:
        key = None
    if _consensus_hist_cache["key"] != key:
        rows: list[dict[str, Any]] = []
        try:
            for line in path.read_text(encoding="utf-8").splitlines():
                ...
                rows.append({
                    ...
                    "reason": c.get("reason"),
                    "agreed": _ag,
                    "primary_text": str(c.get("primary_text") or "")[:80],
                    "secondary_text": str(c.get("secondary_text") or "")[:80],
                    "reviews": d.get("reviews"),
                    "pnl_usd": d.get("total_pnl_usd"),
                })
        except OSError:
            rows = []
        rows.reverse()
        _consensus_hist_cache.update(key=key, rows=rows)
    rows = _consensus_hist_cache["rows"]
    return {"rows": rows[offset:offset + limit], "total": len(rows),
            "offset": offset, "limit": limit}


_today_counts_cache: dict[str, Any] = {"ts": 0.0, "value": {"decisions": 0, "orders": 0}}


def _today_counts() -> dict[str, Any]:
    """Beijing-trading-day decision/order counts for the agent cards, 5s cached."""
    now = time.monotonic()
    if now - _today_counts_cache["ts"] > 5.0:
        try:
            day_cutoff = store.local_day_start_ts()
            with db() as c:
                d = c.execute("SELECT COUNT(*) FROM decisions WHERE ts>=?", (day_cutoff,)).fetchone()[0]
                o = c.execute("SELECT COUNT(*) FROM orders WHERE ts>=?", (day_cutoff,)).fetchone()[0]
            _today_counts_cache["value"] = {"decisions": int(d), "orders": int(o),
                                            "since": day_cutoff}
        except Exception:
            pass
        _today_counts_cache["ts"] = now
    return _today_counts_cache["value"]


_pnl_daily_cache: dict[str, dict] = {}


@app.get("/api/pnl/daily")
def pnl_daily(lane: str = "paper", offset: int = 0, limit: int = 14, asset: str = ""):
    """Per trading-day history with the SAME lot accounting as the headline
    KPIs (store.realized_pnl_summary_today windowed day by day): early exits
    realize their own win/loss lots, so day win rates match what the risk
    gate saw.  Finished days are cached immutably; today recomputes live."""
    lane = lane if lane in ("paper", "live") else "paper"
    offset = max(0, min(int(offset), 3650))
    ...
    try:
        with db() as c:
            row = c.execute(
                "SELECT MIN(x) FROM ("
                "SELECT MIN(r.resolved_ts) AS x FROM resolutions r "
                "JOIN orders o ON o.condition_id=r.condition_id AND o.dry_run=? "
                "UNION ALL "
                "SELECT MIN(ts) FROM orders WHERE dry_run=? AND side LIKE 'SELL_%')",
                (dry_i, dry_i)).fetchone()
        first_ts = float(row[0]) if row and row[0] else None
    except Exception:
        first_ts = None
    today_start = store.local_day_start_ts()
    first_day = store.local_day_start_ts(first_ts) if first_ts else today_start
    span = min(120, int((today_start - first_day) // 86400.0) + 1)
    rows_out: list[dict[str, Any]] = []
    for k in range(span):
        day_start = today_start - k * 86400.0
        key = f"{lane}:{int(day_start)}"
        if k > 0 and key in _pnl_daily_cache:
            summary = _pnl_daily_cache[key]
        else:
            summary = store.realized_pnl_summary_today(
                dry, cutoff=day_start, end=day_start + 86400.0)
            if k > 0:  # finished trading days never change
                _pnl_daily_cache[key] = summary
        if asset:
            bucket = (summary.get("by_asset") or {}).get(asset.upper()) or {}
            wins = int(bucket.get("wins") or 0)
            losses = int(bucket.get("losses") or 0)
            pnl = float(bucket.get("realized_usd") or 0.0)
        else:
            wins = int(summary.get("wins") or 0)
            losses = int(summary.get("losses") or 0)
            pnl = float(summary.get("realized_usd") or 0.0)
        if wins == 0 and losses == 0 and abs(pnl) < 1e-9:
            continue
        rows_out.append({
            "day": time.strftime("%Y-%m-%d", time.gmtime(day_start + store.TRADING_DAY_UTC_OFFSET_HOURS * 3600.0)),
            "pnl_usd": round(pnl, 4), "wins": wins, "losses": losses,
        })
    total_days = len(rows_out)
    rows_out = rows_out[offset:offset + limit]
    if lane == "paper":
        capital = float(getattr(cfg, "paper_equity_usd", 100.0))
    else:
        capital = float(_state.get("balance_pusd") or 0.0)
    return {"lane": lane, "asset": asset.upper(), "rows": rows_out, "total_days": total_days,
            "offset": offset, "limit": limit,
            "capital_base_usd": round(capital, 4)}


@app.get("/api/orders")
def orders_page(offset: int = 0, limit: int = 20, lane: str = "all"):
    """Paginated order history (paper + live), newest first."""
    offset = max(0, min(int(offset), 5000))
    limit = max(1, min(int(limit), 100))
    lane = lane if lane in ("all", "paper", "live") else "all"
    rows = recent_orders(limit=min(5000, offset + limit * 3 + 60))
    if lane == "paper":
        rows = [r for r in rows if r.get("dry_run")]
    elif lane == "live":
        rows = [r for r in rows if not r.get("dry_run")]
    where = {"all": "", "paper": " WHERE dry_run=1", "live": " WHERE dry_run=0"}[lane]
    try:
        with db() as c:
            total = int(c.execute("SELECT COUNT(*) FROM orders" + where).fetchone()[0])
    except Exception:
        total = len(rows)
    return {"rows": rows[offset:offset + limit], "total": total,
            "offset": offset, "limit": limit, "lane": lane}


def _update_env_keys(changes: dict[str, str]) -> None:
    env_path = ROOT / ".env"
    backup = ROOT / f".env.before-risk.{time.strftime('%Y%m%d-%H%M%S')}"
...
_kelly_flag_cache: dict[str, Any] = {"ts": 0.0, "value": False}
...
async def put_risk_settings(request: Request):
    """Operator-gated worker trigger lines; applies to .env and restarts the
    paper workers so the running gate matches what the UI shows."""
    _require_operator(request)
    body = await request.json()
    changes: dict[str, str] = {}
    note = []
    if body.get("max_daily_loss_fraction") is not None:
        loss = max(0.01, min(0.50, float(body["max_daily_loss_fraction"])))
        changes["MAX_DAILY_LOSS_FRACTION"] = f"{loss:g}"
        try:
            cfg.max_daily_loss_fraction = loss
        except Exception:
            object.__setattr__(cfg, "max_daily_loss_fraction", loss)
        note.append(f"日亏损熔断={loss:.0%}")
    if body.get("max_daily_realized_profit_usd") is not None:
        cap = max(0.0, min(10000.0, float(body["max_daily_realized_profit_usd"])))
        changes["MAX_DAILY_REALIZED_PROFIT_USD"] = f"{cap:g}"
        try:
            cfg.max_daily_realized_profit_usd = cap
        except Exception:
            object.__setattr__(cfg, "max_daily_realized_profit_usd", cap)
        note.append("每日止盈上限=" + (f"${cap:g}" if cap > 0 else "关闭"))
    if body.get("paper_equity_usd") is not None:
        equity = max(10.0, min(100000.0, float(body["paper_equity_usd"])))
        changes["PAPER_EQUITY_USD"] = f"{equity:g}"
        try:
            cfg.paper_equity_usd = equity
        except Exception:
            object.__setattr__(cfg, "paper_equity_usd", equity)
        note.append(f"模拟本金=${equity:g}")
    if body.get("kelly_sizing_enabled") is not None:
    ...
    _update_env_keys(changes)
    restarted: list[str] = []
    if bool(body.get("restart_workers", True)):
        import subprocess
        for unit in ("polymarket-auto-btc-paper.service",
                     "polymarket-auto-eth-paper.service"):
            try:
                subprocess.run(["sudo", "-n", "systemctl", "restart", unit],
                               check=True, capture_output=True, timeout=30)
                restarted.append(unit.replace("polymarket-auto-", "").replace(".service", ""))
            except Exception:
                pass
    return {"ok": True, "applied": changes, "workers_restarted": restarted,
            "note": "；".join(note) + ("（worker 已重启生效）" if restarted else "（未重启 worker，重启后生效）")}


@app.websocket("/api/ws/state")
async def ws_state(websocket: WebSocket):
    """Push the shared state frame at a flat ~1s cadence.

    A single producer task (started lazily with the first socket) rebuilds
    the frame; sockets only await the new-frame event and send the cached
    text, so compute time and client count never distort the tick.
    GET /api/state remains the fallback transport."""
    global _ws_producer_task
    await websocket.accept()
    if _ws_producer_task is None or _ws_producer_task.done():
        if _ws_frame["event"] is None:
            _ws_frame["event"] = asyncio.Event()
        _ws_producer_task = asyncio.create_task(_ws_produce_loop())
    try:
        if _ws_frame["text"]:
            await websocket.send_text(_ws_frame["text"])
        while True:
            event = _ws_frame["event"]
            await event.wait()
            await websocket.send_text(_ws_frame["text"])
...
def state():
    ...
    return {
        ...
        "wallet": {
            ...
            "value_usd": _state.get("value_usd") if _state.get("bot_mode") == "live" else (_state.get("value_usd") or _paper_open_stats()[1]),
        ...
        "decisions_total": _ledger_totals()["decisions"],
        "orders_total": _ledger_totals()["orders"],
        "slow_loop": _slow_loop_extras(),
        "feeds": _feeds_status(),
...
# ---- patch11: TTL micro-caches over second-scale snapshots ----------------
# These wrap the module-level names; every call site resolves through module
# globals at runtime, so reassignment here covers state() and the WS producer.
_snap_cache: dict[str, tuple[float, Any]] = {}
_snap_refreshing: set[str] = set()


def _snap_cached(name: str, ttl: float, fn):
    """Stale-while-revalidate: expiry returns the old value instantly and a
    background thread refreshes, so no frame ever pays the snapshot cost."""
    import threading

    def refresh(args, kwargs):
        try:
            value = fn(*args, **kwargs)
            _snap_cache[name] = (time.monotonic(), value)
        except Exception:
            pass  # keep serving the stale value
        finally:
            _snap_refreshing.discard(name)

    def wrapper(*args, **kwargs):
        now = time.monotonic()
        hit = _snap_cache.get(name)
        if hit is None:
            value = fn(*args, **kwargs)  # first call only: compute inline
            _snap_cache[name] = (time.monotonic(), value)
            return value
        if now - hit[0] >= ttl and name not in _snap_refreshing:
            _snap_refreshing.add(name)
            threading.Thread(target=refresh, args=(args, kwargs), daemon=True).start()
        return hit[1]
    return wrapper


_service_status_snapshot = _snap_cached("service", 3.0, _service_status_snapshot)
_agent_status_snapshot = _snap_cached("agents", 2.0, _agent_status_snapshot)
_llm_settings_response = _snap_cached("llm_settings", 10.0, _llm_settings_response)
