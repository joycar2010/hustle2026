...
def wait_healthy(s, want_login="", budget_s=150):
    """等 /health 真 + (可选)账户 login 匹配(MT4 必核: 防陈旧 EA 状态/登录失败假阳性)。"""
    t0 = time.time()
    while time.time() - t0 < budget_s:
        ok, rtt = http_health(s["port"])
        if ok:
            if not want_login:
...
def cell_health():
    with LOCK:
        ...
        alloc = sum(1 for s in SLOTS if s["state"] == "allocated")
    return {"ok": True, "cell": CELL_NAME, "warm": warm, "allocated": alloc, "ts": int(time.time())}
...
def cell_status(x_api_key: str = Header(default="")):
    ...
    for s in SLOTS:
        ...
        ok, rtt = (http_health(s["port"]) if s["state"] == "allocated" else (None, None))
        e["healthy"] = ok; e["rtt_ms"] = rtt
        out.append(e)
