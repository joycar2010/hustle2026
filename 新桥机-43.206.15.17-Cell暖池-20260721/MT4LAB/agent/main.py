...
WAIT_SEC  = float(os.getenv("CMD_WAIT_SEC", "8"))   # 等 EA 结果上限;超时=UNKNOWN 不失败
...
def _submit_and_wait(cmd_id: str, payload: dict):
    ...
    return fb.wait_result(cmd_id, timeout=WAIT_SEC)
...
async def get_positions(symbol: Optional[str] = Query(None)):
    ...
    if not data or not fb.is_fresh(age):
        raise HTTPException(503, "MT4 not connected")
...
async def place_order(req: OrderRequest):
    ...
    if rid:
        ...
        if prev:
            pstate, _ = prev
            if pstate == "DONE":
                r = ledger.get_result(rid) or {}
                r["idempotency_hit"] = True
                return r
            if pstate == "FAILED":
                raise HTTPException(400, "idempotent-replay: previous attempt FAILED terminally")
            # SENDING:结果未知。结果文件已到则落账,否则拒绝重发
            res = fb.read_result(rid)
            if res is None:
                return {"success": False, "unknown": True, "state": pstate, "request_id": rid,
                        "idempotency_hit": True,
                        "error": "previous attempt outcome unknown (SENDING), refuse to resend"}
            ok, payload = _apply_order_result(rid, res)
            if ok:
                payload["idempotency_hit"] = True   # SENDING 靠结果文件解决=幂等命中
                return payload
            raise HTTPException(400, "Order failed: %s" % payload["error"])
        ledger.put(rid, "SENDING")

    cmd_id = rid or uuid.uuid4().hex
    ...
    res = _submit_and_wait(cmd_id, payload)
    if res is None:
        # 超时 = UNKNOWN。命令仍在,EA 迟早执行;QH 走 order-status 查真相,禁止盲重发
        return {"success": False, "unknown": True, "state": "SENDING", "request_id": rid,
                "error": "timeout waiting EA result; outcome unknown, query /mt5/order-status"}
    ok, payload = _apply_order_result(rid, res)
    if ok:
        return payload
    raise HTTPException(400, "Order failed: %s" % payload["error"])
...
async def close_position(req: ClosePositionRequest):
    ...
    res = _submit_and_wait(cmd_id, {"op": "close", "symbol": req.symbol, "side": req.side,
                                    "volume": req.volume, "ticket": req.ticket})
    if res is None:
        raise HTTPException(504, "timeout waiting EA close result; outcome unknown")
    if not res.get("ok"):
        raise HTTPException(400, "Close failed: %s" % (res.get("error") or res.get("retcode")))
    return {"success": True, "retcode": res.get("retcode", 10009), "order": res.get("ticket"),
            "volume": res.get("volume"), "price": res.get("price"), "comment": res.get("comment", "")}
...
async def close_all(req: CloseAllRequest):
    _require_connected()
    cmd_id = uuid.uuid4().hex
    res = _submit_and_wait(cmd_id, {"op": "close_all", "symbol": req.symbol})
    if res is None:
        raise HTTPException(504, "timeout waiting EA close-all result")
    return {"closed": res.get("closed", 0), "failed": res.get("failed", 0),
...
async def cancel_all(req: CancelAllRequest):
    _require_connected()
    cmd_id = uuid.uuid4().hex
    res = _submit_and_wait(cmd_id, {"op": "cancel_all", "symbol": req.symbol})
    if res is None:
        raise HTTPException(504, "timeout waiting EA cancel-all result")
    return {"cancelled": res.get("cancelled", 0), "failed": res.get("failed", 0)}


@app.get("/mt5/order-status/{request_id}", dependencies=[Depends(verify_api_key)])
async def order_status(request_id: str):
    row = ledger.get(request_id)
    if not row:
        raise HTTPException(404, "unknown request_id")
    state, _ = row
    # SENDING 但结果文件已到 → 落账推进(UNKNOWN 解决,QH 超时后调此端点)
    if state == "SENDING":
        res = fb.read_result(request_id)
        if res is not None:
            ok, _ = _apply_order_result(request_id, res)
            state = "DONE" if ok else "FAILED"
    out = {"request_id": request_id, "state": state, "terminal": state in ("DONE", "FAILED")}
    r = ledger.get_result(request_id)
    if r is not None:
        out["result"] = r
    return out
