...
$insts=@(@{n='icsysnew';port=8887;fam='ic'},@{n='icsys';port=8888;fam='ic'},@{n='ic01';port=8021;fam='ic'},@{n='bysys';port=8886;fam='bybit'},@{n='by01';port=8001;fam='bybit'})
function Running($i){ [bool](Get-CimInstance Win32_Process -Filter "Name='python.exe'" | Where-Object { $_.CommandLine -like "*--port $($i.port)*" }) }
function StartB($i){ $dir="D:\QHMT5\instances\$($i.n)"; $py="D:\QHMT5\runtime\releases\v1\venv-$($i.fam)\Scripts\python.exe"; Start-Process -FilePath $py -ArgumentList '-m','uvicorn','app.main:app','--host','0.0.0.0','--port',"$($i.port)" -WorkingDirectory $dir -RedirectStandardError "$dir\logs\bridge.err.log" -RedirectStandardOutput "$dir\logs\bridge.out.log" -WindowStyle Hidden; Log "started $($i.n)" }
...
foreach($i in $insts){ if(-not (Running $i)){ StartB $i; Start-Sleep 15 } else { Log "adopt $($i.n) (already running)" } }
Start-Sleep 60
$bad=@{}; $lastR=@{}
while($true){
  foreach($i in $insts){
