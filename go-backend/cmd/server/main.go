...
import (
	...
	"hustle-go/internal/healthwatch"
)
...
func main() {
	...
	// ── Load MT5 configuration from DB ─────────────────────────────
	mt5config.Init(db.Pool())
	go mt5config.RunReloader(60 * time.Second)
	...
	// ── Market data: multi-exchange WS ─────────────────────────────────
	allPairs := pairs.Global.All()

	// Binance WS (platform 1)
	binanceSymbols := pairs.Global.SymbolsByPlatform(1)
	if len(binanceSymbols) > 0 {
		streamsURL := pairs.Global.BuildBinanceStreamsURL()
		log.Printf("[Server] Binance WS: %s", streamsURL)
		go market.RunBinanceWS(streamsURL)
	}

	// OKX WS (platform 5)
	okxSymbols := pairs.Global.SymbolsByPlatform(5)
	if len(okxSymbols) > 0 {
		log.Printf("[Server] OKX WS: %d symbols %v", len(okxSymbols), okxSymbols)
		go market.RunOKXWS(okxSymbols)
	}

	// Gate.io WS (platform 4)
	gateSymbols := pairs.Global.SymbolsByPlatform(4)
	if len(gateSymbols) > 0 {
		log.Printf("[Server] Gate.io WS: %d symbols %v", len(gateSymbols), gateSymbols)
	...
	// ── Multi-pair pushers ────────────────────────────────────────────
	// TickPusher disabled: causes stale Bybit price in spread calc (only pushes Binance)
	...
	// ── MT5 health watcher ─────────────────────────────────────────
	go healthwatch.Run(db.Redis())
	log.Println("[Server] MT5 HealthWatcher started")
