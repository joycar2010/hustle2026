...
type userRow struct {
	...
	HedgeRatioEnabled bool `json:"hedge_ratio_enabled"`
...
func scanUser(row interface{ Scan(...any) error }) (*userRow, error) {
	...
	return u, row.Scan(
		...
		&u.IsActive, &u.HedgeRatioEnabled, &u.OpenclawEnabled, &u.CreateTime, &u.UpdateTime,
	)
}

const selectUser = `SELECT user_id::text, username, COALESCE(email,''), role,
	feishu_open_id, feishu_mobile, feishu_union_id,
	is_active, COALESCE(hedge_ratio_enabled, false), COALESCE(openclaw_enabled, false), create_time, update_time FROM users`
...
func ListUsers(c *gin.Context) {
	...
	for rows.Next() {
		...
		users = append(users, gin.H{
			...
			"is_active": u.IsActive, "hedge_ratio_enabled": u.HedgeRatioEnabled, "openclaw_enabled": u.OpenclawEnabled, "create_time": u.CreateTime, "update_time": u.UpdateTime,
		})
	}
	if users == nil {
		users = []gin.H{}
