...
// callMT5Bridge calls a specific MT5 bridge directly (not the default MT5_SERVICE_URL).
func callMT5Bridge(ctx context.Context, bridgeURL, method, path string, jsonBody []byte) ([]byte, int) {
	...
	if err != nil {
		return []byte(`{"error":"bridge unreachable"}`), 502
	}
...
func ManualOrder(c *gin.Context) {
	...
	case "binance":
		symbol := "XAUUSDT"
		if pair != nil && pair.APlatformID == PlatformBinance { symbol = pair.ASymbol }
		creds, err := loadCredsByPlatform(ctx, userID, PlatformBinance)
		if err != nil { c.JSON(http.StatusNotFound, gin.H{"detail": "no Binance account"}); return }
		var posSide string
		if req.Side == "buy" { posSide = "LONG" } else { posSide = "SHORT" }
		body, status, err := PlaceBinanceMakerQueueOrder(ctx, creds, symbol, req.Side, posSide, FormatQty(req.Quantity))
		if err != nil { c.JSON(http.StatusBadGateway, gin.H{"detail": err.Error()}); return }
		c.Data(status, "application/json", body)
	case "bybit":
		mt5Sym := "XAUUSD+"
		bURL := ""
		if pair != nil { mt5Sym = pair.MT5Symbol; bURL = pair.BridgeURL }
		payload := map[string]interface{}{"symbol": mt5Sym, "volume": req.Quantity, "order_type": strings.ToUpper(req.Side)}
		b, _ := json.Marshal(payload)
		if bURL != "" {
			rb, st := callMT5Bridge(ctx, bURL, http.MethodPost, "/mt5/order", b)
			c.Data(st, "application/json", rb)
		} else {
			rb, st, err := callMT5(ctx, http.MethodPost, "/mt5/order", b)
			if err != nil { c.JSON(http.StatusBadGateway, gin.H{"detail": err.Error()}); return }
			c.Data(st, "application/json", rb)
		}
	case "okx":
		instId := "XAU-USDT-SWAP"
		if pair != nil && pair.APlatformID == PlatformOKX { instId = pair.ASymbol }
		creds, err := loadCredsByPlatform(ctx, userID, PlatformOKX)
		if err != nil { c.JSON(http.StatusNotFound, gin.H{"detail": "no OKX account"}); return }
...
// Closes ALL positions for the current pair on BOTH sides:
// A-side (Binance/OKX/Gate) uses maker mode, B-side (MT5 bridge) uses taker mode.
func ManualCloseAll(c *gin.Context) {
	...
	pair := pairs.Global.Get(body.PairCode)
	if pair == nil {
		c.JSON(http.StatusBadRequest, gin.H{"detail": "unknown pair_code: " + body.PairCode})
		return
	}

	var results []gin.H

	// ── A-side: close positions (maker mode) ──
	switch pair.APlatformID {
	case PlatformBinance:
		creds, err := loadCredsByPlatform(ctx, userID, PlatformBinance)
		if err == nil {
			posBody, _, err := GetBinancePositions(ctx, creds, pair.ASymbol)
			if err == nil {
				...
				if json.Unmarshal(posBody, &positions) == nil {
					for _, pos := range positions {
						...
						rb, st, _ := PlaceBinanceMakerQueueOrder(ctx, creds, pair.ASymbol,
							side, posSide, FormatQty(amt))
						results = append(results, gin.H{
							"exchange": "binance", "symbol": pair.ASymbol,
							"position_side": posSide, "quantity": amt,
	...
	case PlatformOKX:
		creds, err := loadCredsByPlatform(ctx, userID, PlatformOKX)
		if err == nil {
			okxCreds := &OKXCreds{APIKey: creds.APIKey, Secret: creds.APISecret, Passphrase: creds.Passphrase}
			rb, st, _ := PlaceOKXOrder(ctx, okxCreds, pair.ASymbol, "sell", "market", "0", "")
			results = append(results, gin.H{
				"exchange": "okx", "symbol": pair.ASymbol,
				"status": st, "response": json.RawMessage(rb),
			})
		}
	}
	...
	if pair.BridgeURL != "" {
		url := pair.BridgeURL + "/mt5/position/close-all"
		...
		if req != nil {
			...
			if err == nil {
				...
				rb, _ := io.ReadAll(resp.Body)
				results = append(results, gin.H{
					"exchange": "mt5", "bridge": pair.BridgeURL,
					"symbol": pair.MT5Symbol,
					"status": resp.StatusCode, "response": json.RawMessage(rb),
				})
			}
		}
	} else {
		...
		if err == nil {
			results = append(results, gin.H{
				"exchange": "mt5", "status": st, "response": json.RawMessage(rb),
			})
		}
	}
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"results":   results,
		"message":   "close-all executed for " + body.PairCode,
	})
}
...
func ManualCloseLong(c *gin.Context) {
	userID := c.GetString("user_id")
	var req closePosReq
	if err := c.ShouldBindJSON(&req); err != nil { c.JSON(422, gin.H{"detail": err.Error()}); return }
	pair := pairs.Global.Get(req.PairCode)
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	switch req.Exchange {
	case "binance":
		...
		if pair != nil && pair.APlatformID == PlatformBinance { symbol = pair.ASymbol }
		creds, err := loadCredsByPlatform(ctx, userID, PlatformBinance)
		if err != nil { c.JSON(404, gin.H{"detail": "no Binance account"}); return }
		body, st, err := PlaceBinanceMakerQueueOrder(ctx, creds, symbol, "SELL", "LONG", FormatQty(req.Quantity))
	...
	case "bybit":
		...
		bURL := ""
		if pair != nil { mt5Sym = pair.MT5Symbol; bURL = pair.BridgeURL }
		payload, _ := json.Marshal(map[string]interface{}{"symbol": mt5Sym, "side": "sell", "volume": req.Quantity})
		if bURL != "" {
			rb, st := callMT5Bridge(ctx, bURL, http.MethodPost, "/mt5/position/close", payload)
			c.Data(st, "application/json", rb)
		} else {
			rb, st, err := callMT5(ctx, http.MethodPost, "/mt5/position/close", payload)
			if err != nil { c.JSON(502, gin.H{"detail": err.Error()}); return }
			c.Data(st, "application/json", rb)
		}
	default:
...
func ManualCloseShort(c *gin.Context) {
	...
	case "binance":
		...
		creds, err := loadCredsByPlatform(ctx, userID, PlatformBinance)
		if err != nil { c.JSON(404, gin.H{"detail": "no Binance account"}); return }
		body, st, err := PlaceBinanceMakerQueueOrder(ctx, creds, symbol, "BUY", "SHORT", FormatQty(req.Quantity))
		if err != nil { c.JSON(502, gin.H{"detail": err.Error()}); return }
		c.Data(st, "application/json", body)
	case "bybit":
		mt5Sym := "XAUUSD+"
		bURL := ""
		if pair != nil { mt5Sym = pair.MT5Symbol; bURL = pair.BridgeURL }
		payload, _ := json.Marshal(map[string]interface{}{"symbol": mt5Sym, "side": "buy", "volume": req.Quantity})
		if bURL != "" {
			rb, st := callMT5Bridge(ctx, bURL, http.MethodPost, "/mt5/position/close", payload)
			c.Data(st, "application/json", rb)
		} else {
...
func ManualCancelAll(c *gin.Context) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	var body struct { PairCode string `json:"pair_code"` }
	c.ShouldBindJSON(&body)
	if body.PairCode == "" { body.PairCode = "XAU" }
	pair := pairs.Global.Get(body.PairCode)
	var results []gin.H
	if pair != nil && pair.APlatformID == PlatformOKX {
		if creds, err := loadCredsByPlatform(ctx, userID, PlatformOKX); err == nil {
			okxC := &OKXCreds{APIKey: creds.APIKey, Secret: creds.APISecret, Passphrase: creds.Passphrase}
		...
		symbol := "XAUUSDT"
		if pair != nil { symbol = pair.ASymbol }
		if creds, err := loadCredsByPlatform(ctx, userID, PlatformBinance); err == nil {
			rb, st, _ := CancelAllBinanceOrders(ctx, creds, symbol)
			results = append(results, gin.H{"exchange": "binance", "symbol": symbol, "status": st, "response": json.RawMessage(rb)})
		}
	}
	if pair != nil && pair.BridgeURL != "" {
		rb, st := callMT5Bridge(ctx, pair.BridgeURL, http.MethodPost, "/mt5/cancel-all", nil)
		results = append(results, gin.H{"exchange": "mt5", "status": st, "response": json.RawMessage(rb)})
	} else {
		rb, st, err := callMT5(ctx, http.MethodPost, "/mt5/cancel-all", nil)
		if err == nil { results = append(results, gin.H{"exchange": "mt5", "status": st, "response": json.RawMessage(rb)}) }
	}
	c.JSON(200, gin.H{"success": true, "pair_code": body.PairCode, "results": results, "message": "cancel-all for " + body.PairCode})
}
...
func RealtimeOrders(c *gin.Context) {
	...
	body, status, err := GetBinanceOpenOrders(ctx, creds, symbol)
	...
	for _, o := range raw {
		...
		executedQty, _ := strconv.ParseFloat(mapStr(o["executedQty"]), 64)
...
func RealtimeHistory(c *gin.Context) {
	...
	if creds, err := loadCredsByPlatform(ctx, userID, PlatformBinance); err == nil {
		if body, st, err := GetBinanceUserTrades(ctx, creds, symbol, startMs, endMs, 1000); err == nil && st == 200 {
			binanceTrades = body
		}
		if incBody, st, err := GetBinanceIncome(ctx, creds, symbol, "REALIZED_PNL", startMs, endMs); err == nil && st == 200 {
