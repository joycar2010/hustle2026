...
// ── WebSocket 连接 ───────────────────────────────────────────
let clockSkewMs = 0  // 浏览器与服务器时钟偏移（用于校准延迟计算）
...
function connect() {
  ...
  ws.onmessage = (event) => {
    try {
      ...
      if (msg.timestamp) {
        const now = Date.now()
        const rawLatency = now - msg.timestamp
        // 第一条 connection 消息用作基准（假设此时真实网络延迟接近 0）
        if (msg.type === 'connection' && stats.value.totalMessages === 1) {
          clockSkewMs = rawLatency  // 记录初始偏移
        }
        // 真实延迟 = 原始差值 - 时钟偏移
        const latency = Math.max(0, rawLatency - clockSkewMs)
  ...
  try {
    ...
    for (const ep of endpoints) {
      try {
        ...
        if (r.data) {
          const d = r.data
          // Normalize Go ws/stats format: {connections:{total:N}} → flat
          serverStats.value = {
            connections: d.connections?.total ?? d.ws_connections ?? d.connections ?? 0,
            total_connections: d.total_connections ?? d.connections?.total ?? 0,
            messages_today: d.messages_today ?? d.total_messages ?? '--',
            pending_messages: d.pending_messages ?? 0,
            status: d.service ? 'ok' : (d.healthy ? 'ok' : '--'),
            service: d.service || '--',
          }
          break
        }
