<template>
  <div class="container mx-auto px-4 py-6 space-y-6">
    ...
    <!-- ===== 产品对配置状态 ===== -->
    <div class="space-y-2">
      <h2 class="text-sm font-semibold text-text-tertiary uppercase tracking-wider px-1">产品对配置状态</h2>
      <div class="bg-dark-100 rounded-xl border border-border-primary p-4">
        <div v-if="hedgingPairs.length" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5 gap-3">
          <div v-for="pair in hedgingPairs" :key="pair.pair_code"
            class="bg-dark-200 rounded-lg px-3 py-2.5 text-xs border"
            ...
            <div class="text-text-tertiary space-y-0.5">
              <div class="flex justify-between">
                <span>CEX 符号</span>
                <span class="font-mono text-text-secondary">{{ pair.symbol_a?.symbol || '--' }}</span>
              </div>
              <div class="flex justify-between">
                <span>MT5 符号</span>
                <span class="font-mono text-text-secondary">{{ pair.symbol_b?.symbol || '--' }}</span>
              </div>
              <div class="flex justify-between">
                <span>转换因子</span>
                <span class="font-mono text-primary font-bold">{{ pair.conversion_factor || '--' }}</span>
              </div>
              <div class="flex justify-between">
                <span>平台</span>
                <span class="text-text-secondary">{{ pair.platform_a?.platform_name || 'Binance' }} / {{ pair.platform_b?.platform_name || 'MT5' }}</span>
              </div>
            </div>
          </div>
        </div>
        <div v-else class="text-center text-text-tertiary text-sm py-4">
          {{ hedgingPairsLoading ? '加载中...' : '暂无产品对配置' }}
        </div>
      </div>
    </div>
          ...
          <div v-if="monitorData.mt5_clients?.length" class="space-y-2">
            <div v-for="c in monitorData.mt5_clients" :key="c.mt5_login"
              ...
              <div class="flex items-center gap-1.5">
                ...
                <span v-if="c.is_system_service" class="px-1 py-0.5 rounded text-[10px] bg-purple-500/15 text-purple-400 leading-none">系统服务</span>
                <span class="text-text-tertiary">{{ c.username }}</span>
              </div>
              <div class="flex items-center gap-2">
...
const hedgingPairsLoading = ref(false)
...
async function fetchHedgingPairs() {
  hedgingPairsLoading.value = true
  try {
    ...
    hedgingPairsLoading.value = false
  }
}
