        ...
        <div class="overflow-x-auto">
          <table class="w-full text-xs md:text-sm">
            ...
            <tbody>
              ...
              <tr v-for="o in orders" :key="o.id || o.order_id || o.ticket" class="border-b border-border-secondary hover:bg-dark-50 transition-colors">
                <td class="px-4 py-2.5 font-mono text-text-tertiary whitespace-nowrap">{{ fmtTime(o.created_at || o.filled_at || o.time) }}</td>
                <td class="px-3 py-2.5 text-text-secondary">{{ o.account_name || '--' }}</td>
                <td class="px-3 py-2.5 font-medium">{{ o.symbol }}</td>
                <td class="px-3 py-2.5">
                  <span :class="isBuy(o.side) ? 'text-success' : 'text-danger'" class="font-semibold">{{ o.side }}</span>
                </td>
                <td class="px-3 py-2.5 text-right font-mono text-text-secondary">{{ o.quantity || o.filled_quantity || o.volume || '--' }}</td>
                <td class="px-3 py-2.5 text-right font-mono text-text-secondary">{{ o.price ? parseFloat(o.price).toFixed(2) : '--' }}</td>
                <td class="px-4 py-2.5 text-right font-mono font-semibold" :class="pnlColor(o.pnl || o.realized_pnl || o.profit)">
                  {{ pnlStr(o.pnl || o.realized_pnl || o.profit) }}
                </td>
              </tr>
            </tbody>
          </table>
...
const statsCards = computed(() => {
  const pnls = orders.value.map(o => parseFloat(o.pnl || o.realized_pnl || o.profit || 0))
  ...
  return [
    ...
    { label: '胜率',         value: rate + '%', color: parseFloat(rate) >= 50 ? 'text-success' : 'text-warning', sub: wins + ' 盈 / ' + losses + ' 亏' },
    { label: '最大单笔盈亏', value: pnls.length ? (Math.max(...pnls) >= 0 ? '+' : '') + Math.max(...pnls).toFixed(2) : '--', color: 'text-primary' },
  ]
})
...
/**
 * 查询用户所有账户的交易历史
 * 优先使用 /trading/history/realtime (Python, 聚合所有账户)
 * 回退到 /orders/history, 再回退到 /trading/history
 */
async function queryData() {
  loading.value = true
  const params = { start_time: startTime.value + ':00', end_time: endTime.value + ':00' }
  try {
    // 优先: Python 实时交易历史接口（聚合所有账户）
    const r = await api.get('/api/v1/trading/history/realtime', { params })
    const d = r.data
    orders.value = parseOrders(d)
  } catch {
    try {
      const r2 = await api.get('/api/v1/orders/history', { params })
      orders.value = parseOrders(r2.data)
    } catch {
      try {
        const r3 = await api.get('/api/v1/trading/history', { params })
        orders.value = parseOrders(r3.data)
      } catch { orders.value = [] }
    }
...
function parseOrders(d) {
  ...
  if (d?.orders) return d.orders
  if (d?.records) return d.records
  if (d?.trades) return d.trades
  return []
}
