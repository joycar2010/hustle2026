"""每日对账+汇总,推飞书。cron 触发: python3 daily_recon.py"""
...
wbtc=rpc.erc20_balance(WBTC,W)/1e8; usdc=rpc.erc20_balance(ch.stable,W)/1e6; eth=rpc.eth_balance(W)/1e18
...
# 两腿匹配检查
mismatch = abs(wbtc - short_amt) > 0.0005 and (wbtc>0 or short_amt>0)
# 当日 exec_log 统计
...
L=[f'📋 CrossArb P1 日报 {bj}(北京)','',
   f'【持仓对账】{"⚠不匹配!" if mismatch else "✓两腿匹配"}',
   f'· 链上WBTC {wbtc:.6f} | 币安空头 {short_amt:.6f}',
   f'· USDC {usdc:.2f} | ETH(gas) {eth:.5f}',
   '',
   f'【今日成交】',
   f'· 成交 {ok} 笔 | 裸腿 {naked} | 停机 {halt}',
   f'· 平均兑现net {avg_net:.1f}bps | 平均捕获率 {avg_cap*100:.0f}%' if ok else '· 今日暂无成交(基差未达25bps)']
if eth<0.001: L.append('⚠ gas余额偏低,需补ETH')
text='\n'.join(L)
# 发飞书
