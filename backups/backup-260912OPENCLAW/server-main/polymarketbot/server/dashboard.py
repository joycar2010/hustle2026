...
def _service_status_snapshot() -> dict[str, Any]:
    ...
    policy = {
        ...
        "ETH": {"live_enabled": False if auto else eth_enabled, "can_enable_live": not auto, "mode": process_mode if assets["ETH"] and process_mode in ("live", "paper") and not auto else "paper", "running": assets["ETH"]},
    ...
    if auto:
        # A copied .env must never make non-BTC services appear live.
        for service in ("ETH", "WEATHER", "SPORTS"):
...
def _agent_status_snapshot() -> dict[str, Any]:
    ...
    for agent in OPENCLAW_AGENT_IDS:
        status = "idle" if running else "offline"
        if running and agent in ("desk", "risk"):
            status = "running"
        if agent == "sniper" and paper_mode:
            status = "blocked"
        result[agent] = {"status": status, "heartbeat_ts": time.time() if running else None, "queue_depth": 0, "market": "BTC/ETH", "mode": "paper" if paper_mode else "live", "detail": "Paper 服务；未连接 OpenClaw 执行进程" if paper_mode else "由策略服务驱动"}
    return result
...
def live_service_toggle(asset: str, payload: LiveTogglePayload, request: Request):
    """Set an independent live gate; only BTC is eligible in polyauto."""
    _require_operator(request)
    service = _service_key(asset)
    if _is_polyauto_instance() and service != "BTC" and payload.live_enabled:
        raise HTTPException(status_code=403, detail=f"polyauto {service} 实盘路径已永久锁定；仅允许 BTC")
...
def live_service_start(asset: str, request: Request):
    service = _service_key(asset)
    if _is_polyauto_instance() and service != "BTC":
        raise HTTPException(status_code=403, detail="polyauto 仅允许 BTC 实盘")
    if service != "BTC":
...
def live_start(request: Request):
    ...
    with _live_control_lock:
        ...
        launcher = "run_live_btc.sh" if _is_polyauto_instance() else "run_live.sh"
        try:
...
def _llm_advisor_call(slot: str, prompt: str) -> dict[str, Any]:
    ...
    body = {"model": model, "messages": [{"role": "system", "content": "你是只读量化顾问。只输出分析建议，不执行交易、工具或资金操作。"}, {"role": "user", "content": prompt}], "temperature": float(values.get(prefix + "TEMPERATURE", "0.2") or 0.2), "max_tokens": int(float(values.get(prefix + "MAX_TOKENS", "512") or 512))}
    last: Exception | None = None
    for candidate in _model_aliases(values.get(prefix + "PROVIDER", ""), model):
        body["model"] = candidate
        try:
