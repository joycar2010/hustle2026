...
def main() -> int:
    ...
    if sig_type != "3" or funder.lower() == wallet.lower():
        ...
        if len(code) == 0:
            ...
            # New Polymarket Deposit Wallets may use EIP-7702 delegated EOA
            # code (0xef0100 + implementation address), so owner() is not a
            # reliable probe and the old proxy-wallet check would false-fail.
            delegated_7702 = code.hex().lower().startswith("ef0100") and len(code) == 23
            if delegated_7702:
                print(f"{OK} deposit wallet: {deposit} (EIP-7702 delegated wallet)")
