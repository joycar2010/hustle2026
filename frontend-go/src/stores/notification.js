...
export const useNotificationStore = defineStore('notification', () => {
  ...
  // Load alert settings from backend — accepts optional pairCode for per-pair thresholds
  async function loadAlertSettings(pairCode = 'XAU') {
    try {
  ...
  async function playAlertSound(alert) {
    ...
    try {
      ...
      // Construct full URL:
      // - DB stores bare filenames: "spread.mp3" → serve from /sounds/spread.mp3
      // - /uploads/... paths use same-origin with optional VITE_API_BASE_URL
      // - Already-absolute paths (/sounds/...) are used as-is
      const apiBaseUrl = import.meta.env.VITE_API_BASE_URL ?? ''
      let soundUrl
      if (soundFile.startsWith('/')) {
        // Already absolute path (e.g. /sounds/hello-moto.mp3)
        soundUrl = soundFile
      } else if (soundFile.startsWith('http')) {
        // External URL
        soundUrl = soundFile
      } else {
        // Bare filename from DB (e.g. "spread.mp3") → /sounds/spread.mp3
        soundUrl = `/sounds/${soundFile}`
      }
      ...
      for (let i = 0; i < repeatCount; i++) {
        try {
          ...
          // canplaythrough is unreliable in Chrome when resource is cached.
          // We use a race: resolve immediately on canplay/canplaythrough/loadeddata,
          // and fall back after 2 s so a stuck event never blocks playback.
          await new Promise((resolve) => {
            ...
            // safety timeout — if none of the above fires within 2 s, proceed anyway
            setTimeout(ok, 2000)
            audio.load()
          })
  ...
  function handleRiskAlert(alertData) {
    const alertType = alertData.alert_type || 'risk_alert'

    // Single-leg alerts are gated by the Navbar 单腿提醒 switch.
    // Backend now publishes single-leg alerts as risk_alert with alert_type=single_leg_alert,
    // so we honour the same toggle here that checkSingleLegAlert() honours.
    if (alertType === 'single_leg_alert' && !singleLegAlertEnabled.value) {
      console.log('[notification] single-leg alert suppressed by user toggle')
      return
    }

    // Pop-up display prefers the dedicated 弹窗内容模板 (popup_config) over the
    // 飞书内容模板 (alertData.title/message). The backend renders both server-side:
    //   popup_config.title   ← popup_title_template (or fallback to title_template)
    //   popup_config.content ← popup_content_template (or fallback to content_template)
    // Using popup_config here ensures the admin's "弹窗内容模板" edit actually takes effect.
    const popupCfg = alertData.popup_config || {}
    const newAlert = {
      id: Date.now(),
      type: alertType,
      level: alertData.level || 'warning',
      title: popupCfg.title || alertData.title || '风险警报',
      message: popupCfg.content || alertData.message,
      timestamp: alertData.timestamp || new Date().toISOString(),
      // 添加弹窗配置 (kept for sound playback in playAlertSound)
