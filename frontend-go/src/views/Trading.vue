  ...
  <div class="container mx-auto px-4 py-6">
    ...
    <div class="card mb-6">
      <div class="flex flex-wrap items-center gap-4">
        ...
        <!-- Export Dropdown — 任意一侧有数据即可导出 -->
        <div class="relative" v-if="hasAnyData">
    ...
    <div v-if="pairedTrades.length > 0" class="card mb-6">
        ...
        <table class="w-full text-xs">
          <thead>
            <tr class="text-left text-gray-400 border-b border-gray-700">
              ...
              <th class="pb-2 px-1 text-gray-500">滑点(触发)</th>
          ...
          <tbody>
            ...
            <tr v-for="p in filteredPairedTrades" :key="p.id" class="border-b border-gray-800 hover:bg-dark-200">
              ...
              <td class="py-2 px-1 font-mono">
                <span v-if="p.source === 'strategy' && p.slippage_vs_trigger != null"
                      :class="p.slippage_vs_trigger <= 0 ? 'text-green-600' : 'text-red-400'"
                      :title="p.trigger_spread != null ? ('触发价差=' + Number(p.trigger_spread).toFixed(2)) : ''">
                  {{ p.slippage_vs_trigger > 0 ? '+' : '' }}{{ Number(p.slippage_vs_trigger).toFixed(2) }}
                </span>
                <span v-else class="text-gray-600">-</span>
              </td>
              <td class="py-2 px-1 font-mono">
      ...
      <div class="mt-2 text-xs text-gray-500">
        <span>说明: 价差=方向化实得价差(反向=主-对冲,正向=对冲-主); 滑点=实得价差相对设定阈值的偏离(=价差−阈值,开/平符号化),更有利为负(绿)、更差为正(红),与实时滑点护栏同口径; 滑点(触发)=相对触发瞬时真实价差的纯执行滑点(审计用,悬停看触发价差); 配对按主账号maker成交因果FIFO匹配对冲taker成交. 手动交易不展示.</span>
      </div>
    </div>
    ...
    <div class="card mb-6">
      <!-- ① 两侧都无数据 -->
      <div v-if="!hasBinanceData && !hasMt5Data" class="text-center py-12">
        <div class="text-gray-400 text-lg mb-2">暂无交易数据</div>
        <div class="text-gray-500 text-sm">请选择日期查询，或点击最近1天/7天/30天快速查询</div>
      </div>

      <!-- ② 单腿：只有主账号，MT5无成交 -->
      <div v-else-if="hasBinanceData && !hasMt5Data" class="text-center py-10">
        <div class="text-yellow-400 text-lg mb-2 font-semibold">⚠ 单腿记录：该时段仅主账号有成交</div>
        <div class="text-gray-400 text-sm mb-4">
          对冲账户（{{ currentPair }}）在此时间段无成交，不构成完整套利记录，数据不显示
        </div>
        <div class="text-gray-500 text-xs">主账号成交: {{ accountTrades.length }} 条 / 对冲账户成交: 0 条</div>
      </div>

      <!-- ③ 单腿：只有MT5，主账号无成交 -->
      <div v-else-if="!hasBinanceData && hasMt5Data" class="text-center py-10">
        <div class="text-yellow-400 text-lg mb-2 font-semibold">⚠ 单腿记录：该时段仅对冲账户有成交</div>
        <div class="text-gray-400 text-sm mb-4">
          主账号（{{ currentPair }}）在此时间段无成交，不构成完整套利记录，数据不显示
        </div>
        <div class="text-gray-500 text-xs">主账号成交: 0 条 / 对冲账户成交: {{ mt5Trades.length }} 条</div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="bg-gray-800 p-3 rounded col-span-2 grid grid-cols-2 gap-4">
              <!-- 资金费汇总（主账号 Binance 永续合约资金费） -->
              <div>
                <div class="text-xs text-gray-400 mb-1">资金费汇总</div>
                <div class="text-lg font-bold" :class="(stats.fundingFee || 0) >= 0 ? 'text-green-500' : 'text-red-500'">
                  {{ (stats.fundingFee || 0) >= 0 ? '+' : '' }}{{ (stats.fundingFee || 0).toFixed(4) }} USDT
                </div>
              </div>
              <!-- 已实现盈亏 -->
              <div>
    ...
    <!-- Trading History Tables — 仅在两侧都有数据时显示 -->
    <div v-if="hasData" class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              <tr v-for="trade in accountTrades" :key="trade.id" class="border-b border-gray-800">
                ...
                <td class="text-red-500 text-xs">
                  <template v-if="trade.commission_asset === 'BNB' && trade.fee_bnb">
...
const stats = ref({
  ...
  totalFees: 0,
  realizedPnL: 0,
  fundingFee: 0,        // Binance 资金费汇总（FUNDING_FEE）
  overnightFees: 0,
  marketFundingRate: 0,
  mt5OvernightFee: 0,
  mt5Volume: 0,
...
// 对冲账户（MT5 Bridge）是否有成交
const hasMt5Data = computed(() => mt5Trades.value.length > 0)
// 两侧都有数据 → 完整套利记录
const hasData = computed(() => hasBinanceData.value && hasMt5Data.value)
// 任意一侧有数据 → 已查询过（用于显示导出按钮）
const hasAnyData = computed(() => hasBinanceData.value || hasMt5Data.value)
...
// Re-query when pair changes (auto-refresh with current time range)
watch(currentPair, () => {
  if (startTime.value && endTime.value) queryData()
})
</script>
