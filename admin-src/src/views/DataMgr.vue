  ...
  <div>
    <el-tabs v-model="tab" type="border-card">
      ...
      <el-tab-pane label="版本管理" name="version">
        ...
        <div style="color:#909399;font-size:12px;margin-top:8px">QH 靠 patch 脚本部署,此处仅只读展示,不提供 git 推送/回滚(部署模型不匹配)。</div>
      </el-tab-pane>
      ...
      <!-- WS/心跳 -->
      <el-tab-pane label="WS/心跳监控" name="ws">
        <div style="margin-bottom:8px"><el-button size="small" @click="loadWs">刷新</el-button></div>
        <el-descriptions :column="2" border size="small" v-if="ws">
          <el-descriptions-item label="引擎循环">{{ws.engine&&ws.engine.cycle||'—'}}</el-descriptions-item>
          <el-descriptions-item label="行情心跳">{{ws.engine&&ws.engine.market||'—'}}</el-descriptions-item>
          <el-descriptions-item label="自动进场last">{{ws.engine&&ws.engine.last||'—'}}</el-descriptions-item>
          <el-descriptions-item label="DB连接数">{{ws.db_connections}}</el-descriptions-item>
          <el-descriptions-item label="Redis客户端">{{ws.redis&&ws.redis.connected_clients}}</el-descriptions-item>
          <el-descriptions-item label="Redis内存">{{ws.redis&&ws.redis.used_memory_human}}</el-descriptions-item>
          <el-descriptions-item label="Redis运行(秒)">{{ws.redis&&ws.redis.uptime_sec}}</el-descriptions-item>
        </el-descriptions>
        <div style="color:#909399;font-size:12px;margin-top:8px">QH 无独立 WS hub,此面板读 Redis 引擎心跳 + 连接数(只读)。</div>
      </el-tab-pane>
    </el-tabs>

    <el-dialog v-model="tblDlg" :title="'表数据 · '+tblName" width="80%">
    ...
    <el-dialog v-model="sslDlg" title="上传 SSL 证书" width="560">
