  ...
  <div class="layout">
    ...
    <div class="main-wrap">
      <div class="topbar">
        ...
        <span style="font-weight:600;color:#2E8BD6">管理后台</span>
        <span class="sp"></span>
        ...
        <el-dropdown trigger="click" style="margin:0 4px">
          <span style="cursor:pointer;font-size:13px;color:#2E8BD6;font-weight:600">👤 {{op.operator||'超级管理员'}} ({{op.role||'super'}}) ▾</span>
      ...
      <div class="tabs-bar">
        <el-tabs v-model="activeTab" type="card" closable @tab-remove="removeTab" @tab-click="clickTab" style="--el-tabs-header-height:32px">
          <el-tab-pane v-for="tab in tabs" :key="tab.path" :label="t(tab.title)" :name="tab.path"/>
        </el-tabs>
      </div>
      <div class="crumb">管理后台 / {{ t($route.meta.title||'dashboard') }}</div>
    ...
    <div v-if="!authed" class="login-gate">
      <div class="login-card">
        <div class="lg-logo">QUANT HEDGE 管理后台</div>
        ...
        <el-tabs v-model="gateTab" stretch>
          <el-tab-pane label="操作员登录" name="op">
            ...
            <el-input v-model="opForm.password" type="password" placeholder="密码" show-password @keyup.enter="gateOpLogin"/>
            <el-button type="primary" style="width:100%;margin-top:14px" @click="gateOpLogin">登 录</el-button>
          </el-tab-pane>
          <el-tab-pane label="超管令牌" name="admin">
            <el-input v-model="gateToken" type="password" placeholder="Admin Token" show-password style="margin-bottom:10px" @keyup.enter="gateAdminLogin"/>
