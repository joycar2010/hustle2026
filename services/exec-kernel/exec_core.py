...
class SagaExecutor:
    ...
    async def reduce_pair(self, saga_id, legs, fraction, step_ns=None):
        """分片减仓(§8.6 RiskExitSaga 减险原语):所有腿同比例 reduce-only 平仓。
        任一腿减不掉 → QUARANTINED 人工(减险路径宁隔离不留敞口失衡)。
        step_ns 区分多轮分片(reduce-1/reduce-2..)使 clientOrderId 每轮唯一(哈希尾课已在 adapter 收编)。
        不改 Saga 主态(仍 OPEN),只登记减量;全清由 close_pair 收尾。"""
