...
async def list_accounts(
    ...
    result = await db.execute(
        select(Account).where(Account.user_id == UUID(user_id), Account.is_active == True)
    )
...
async def get_aggregated_dashboard(
    ...
    try:
        aggregated_data = await account_data_service.get_aggregated_account_data(list(accounts))
