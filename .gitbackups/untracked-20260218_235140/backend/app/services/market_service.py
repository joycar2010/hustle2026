...
class MarketDataService:
    ...
    async def get_bybit_quote(
        ...
        """Get Bybit market quote

        Note: For MT5 accounts, symbol might be XAUUSD or XAUUSD.s
        """
        try:
            data = await self.bybit_client.get_tickers(category, symbol)
    ...
    async def get_spread_history(
        ...
        """Get historical spread data from Redis"""
        redis_client = await self._get_redis()
        history_key = f"spread_history:{binance_symbol}:{bybit_symbol}"

        # Get from sorted set (timestamp as score)
        data = await redis_client.zrevrange(history_key, 0, limit - 1)
