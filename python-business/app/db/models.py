from sqlalchemy import (
    Column, Integer, String, Boolean, Numeric, DateTime, Text, ForeignKey,
...
class Symbol(Base):
    ...
    is_new_coin = Column(Boolean, default=False)
    is_delisting = Column(Boolean, default=False)
    allow_open = Column(Boolean, default=True)
...
class SubAccount(Base):
    ...
    api_key = Column(EncryptedString(200), nullable=False)
    api_secret = Column(EncryptedString(400), nullable=False)
...
class GlobalRules(Base):
    ...
    # stop-loss: max unrealized loss (USDT) per position before forced close
    max_loss_per_position = Column(Numeric(15, 2), nullable=True)
    # circuit breaker: spread change threshold (%) to pause opening
    circuit_breaker_spread_pct = Column(Numeric(10, 4), nullable=True)
    circuit_breaker_pause_sec = Column(Integer, default=300)
    # daily interest rate cap (e.g. 0.05 = 5% daily rate cap)
    max_daily_interest_rate = Column(Numeric(10, 6), nullable=True)
    # repay spread: independent threshold for triggering repay close
...
class SymbolRule(Base):
    ...
    allow_remove = Column(Boolean, default=True)
    allow_repay = Column(Boolean, default=True)
    max_daily_interest_rate = Column(Numeric(10, 6), nullable=True)
    repay_spread = Column(Numeric(10, 4), nullable=True)
...
class AccountSymbolRule(Base):
    ...
    __table_args__ = (
        UniqueConstraint("sub_account_id", "symbol", name="uq_account_symbol"),
    )
...
# Import all models into same Base so create_all() covers them
from engine.models import Position, TradeLog, EngineState  # noqa: E402, F401
from app.db.models_auth import User, AdminUser, AuditLog  # noqa: E402, F401
