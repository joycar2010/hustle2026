...
def _winning_token_for(cfg: Config, market_slug: str) -> Optional[str]:
    try:
        ...
        # A resolved binary market must have a decisive, finite outcome. Do
        # not turn a tie, malformed response, or in-progress prices into a
        # false winner and an incorrect PnL record.
        if any(not (0.0 <= p <= 1.0) for p in prices) or abs(prices[0] - prices[1]) < 0.5:
            return None
