...
# ---------------- 1) 点差闸 (与自动/手动入场策略同一口径) ----------------
def spread_gate(main_ask, main_bid, hedge_ask, hedge_bid, entry_spread_thr, direction="reverse"):
    """返回 (可入场?, 有符号方向点差, 原因)。入场阈值是下限: 当前点差 >= 阈值才通过。"""
    if None in (main_ask, main_bid, hedge_ask, hedge_bid):
        return False, None, "quote_missing"
    if direction not in ("reverse", "forward"):
        return False, None, "direction_invalid"
    spread = round((hedge_ask - main_bid) if direction == "reverse" else (main_ask - hedge_bid), 5)
    threshold = float(entry_spread_thr or 0)
    if threshold > 0 and spread < threshold:
        return False, spread, "spread_below_entry(%.5f<%.2f)" % (spread, threshold)
...
# ---- V1.1 移植: 行情新鲜度闸(纯函数) ----
# 病灶: 桥半开/报价冻结时, tick 仍能返回但 time 停走 → 引擎拿陈旧价开仓(pairscan 实测"报价冻结假信号"存在)。
# 用法: 开仓决策前对双腿 tick 调 quote_stale_reason, 返回非 None 即 fail-closed 跳过/拒绝。
def quote_age(tick, broker_off=0, now_ts=None):
    """单腿 tick 源龄(秒)。tick.time/time_msc=经纪商墙钟 epoch(桥端原样), 减 broker_off(活标定偏移,秒)
       折回真UTC后与本机UTC比。优先毫秒 time_msc。返回 None=无法度量(缺time/非数字), 上层放行不误伤
       —— 冻结报价仍带旧 time, 一定能被逮住; 度量不了的是另一类异常(如云端回退 tick 的 ISO 字符串)。"""
    if not isinstance(tick, dict): return None
    # A2T 云端回退 tick(src="a2t") 豁免: 其 time 为各经纪商墙钟(实测 Exness=UTC / ICM demo=GMT+3, 逐腿不一),
    # 与桥标定的单一 broker_off 语义不符 → 强行度量会对时区不同的腿恒判陈旧(误拦纯云端托管用户)。
    # 本闸只防"桥半开/报价冻结"(原始病灶), 桥 tick 语义明确才度量。
    if tick.get("src") == "a2t": return None
    import time as _t
    now = _t.time() if now_ts is None else float(now_ts)
    ts = None
    tm = tick.get("time_msc")
    ...
    return max(0.0, now - (ts - float(broker_off or 0)))

def quote_stale_reason(main_tick, hedge_tick, broker_off=0, max_age_sec=15.0, now_ts=None):
    """双腿新鲜度闸: 任一腿源龄>阈值 → 返回原因串(供 fail-closed 拒开仓); 否则 None。
       max_age_sec<=0 = 闸关闭。双桥墙钟已实测到秒级一致(IC/Bybit 同 GMT+3), 共用单一 broker_off。"""
    try: lim = float(max_age_sec or 0)
    except Exception: lim = 0.0
    if lim <= 0: return None
    for name, tk in (("主", main_tick), ("对冲", hedge_tick)):
        age = quote_age(tk, broker_off, now_ts)
