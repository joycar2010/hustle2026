param(
    [string]$Stage = "D:\QHMT5\deploy\mt5-execution-quote-snapshot-20260821.1-deploy",
...
$AppInitPath = Join-Path $AppRoot "__init__.py"
$CellRoot = "D:\QHCELL"
...
$Expected = @{}
$ProductionBefore = @{}
$ProductionBeforeInitPresent = $false
$Manifest = $null
$Instances = @(
...
function Assert-ReleaseManifest {
    ...
    $requiredCandidateNames = @("__init__.py", "cell.py", "deploy.ps1", "main.py", "runtime.py")
    ...
    $script:ProductionBeforeInitPresent = (
        $Manifest.production_before.app_init_present -eq $true
    )
    if ($ProductionBeforeInitPresent) {
        $script:ProductionBefore["__init__.py"] = Get-ManifestHash `
            $Manifest.production_before "__init__.py"
    }
    ...
    Assert-FileHash (Join-Path $Stage "app\__init__.py") $Expected["__init__.py"]
...
function Copy-DurableDatabase([string]$SourceDb, [string]$TargetDb, [string]$BackupPrefix) {
    ...
    # A WAL database is one durable unit. A failed prior cutover may have
    # already created the fenced target set before rolling code back. Reuse
    # that set after validating its presence; never overwrite it or roll it
    # back, because it may contain newer broker evidence.
    $reuseTarget = @{}
    ...
    try {
        foreach ($entry in $entries) {
            ...
            $entrySuffix = $entry.Target.Substring($TargetDb.Length)
            if (-not $samePath -and -not $reuseTarget[$entrySuffix]) {
        ...
        foreach ($suffix in $suffixes) {
            ...
            if (-not $samePath -and -not $reuseTarget[$suffix] -and
                    (Test-Path -LiteralPath ($TargetDb + $suffix))) {
                throw "target durable database appeared during migration: $($TargetDb + $suffix)"
            }
            if (-not $samePath -and $reuseTarget[$suffix] -and
                    -not (Test-Path -LiteralPath ($TargetDb + $suffix))) {
                throw "reused durable database disappeared during migration: $($TargetDb + $suffix)"
            }
        }
        foreach ($entry in $entries) {
        ...
        if (-not $samePath) {
            foreach ($entry in $entries) {
                if (-not $reuseTarget[$entry.Target.Substring($TargetDb.Length)]) {
...
function Assert-ProductionBeforeFiles {
    ...
    if ($ProductionBeforeInitPresent) {
        Assert-FileHash $AppInitPath $ProductionBefore["__init__.py"]
    } elseif (Test-Path -LiteralPath $AppInitPath) {
        throw "production app package drift: expected __init__.py to be absent"
    }
}
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        try {
            ...
            if ($health.build_id -eq $Release -and $health.mt5 -eq $true -and
                $health.trade_allowed -ne $false -and
...
function Restore-BackupFiles {
    ...
    if ($ProductionBeforeInitPresent) {
        Copy-Item -LiteralPath (Join-Path $Backup "__init__.py.before") -Destination $AppInitPath -Force
    } elseif (Test-Path -LiteralPath $AppInitPath) {
        Remove-Item -LiteralPath $AppInitPath -Force
    }
    Assert-FileHash (Join-Path $AppRoot "main.py") $ProductionBefore["main.py"]
    Assert-FileHash (Join-Path $AppRoot "runtime.py") $ProductionBefore["runtime.py"]
    Assert-FileHash (Join-Path $CellRoot "cell.py") $ProductionBefore["cell.py"]
...
try {
    ...
    if ($ProductionBeforeInitPresent) {
        Copy-Item -LiteralPath $AppInitPath -Destination (Join-Path $Backup "__init__.py.before")
    }
    ...
    if ($ProductionBeforeInitPresent) {
        Assert-FileHash (Join-Path $Backup "__init__.py.before") $ProductionBefore["__init__.py"]
    }
    ...
    Copy-Item -LiteralPath (Join-Path $Stage "app\__init__.py") -Destination $AppInitPath -Force
    ...
    Assert-FileHash $AppInitPath $Expected["__init__.py"]
    ...
    foreach ($instance in $Instances) {
        ...
        Set-EnvValue $envPath "MT5_PUBLIC_TICK_CACHE_TTL_MS" "250"
        Set-EnvValue $envPath "MT5_NATIVE_TICK_CACHE_TTL_MS" "300"
        Set-EnvValue $envPath "MT5_TRADE_QUOTE_MODE" "lazy"
        Set-EnvValue $envPath "MT5_CLOSE_SNAPSHOT_MAX_AGE_MS" "500"
        Set-EnvValue $envPath "MT5_REQUOTE_RETRIES" "1"
        Set-EnvValue $envPath "SYMBOL_INFO_CACHE_TTL_MS" "1000"
        Set-EnvValue $envPath "POST_TRADE_SNAPSHOT_GRACE_MS" "50"
    ...
    foreach ($instance in $Instances) {
        ...
        $executionProbe = Invoke-Bridge $instance `
            "/mt5/connection/status?execution_probe=true" 5
        $expectedAccount = Get-EnvValue `
            (Join-Path $instance.Root ".env") "MT5_LOGIN"
        if ($executionProbe.execution_probe -ne $true -or
                $executionProbe.execution_health_source -ne "memory" -or
                $executionProbe.execution_healthy -ne $true -or
                $executionProbe.connected -ne $true -or
                $executionProbe.healthy -ne $true -or
                $executionProbe.account_identity_verified -ne $true -or
                [string]$executionProbe.account -ne [string]$expectedAccount) {
            throw "memory execution probe failed: $($instance.Name)"
        }
        ...
        $healthRows += @{
            ...
            execution_probe_source = $executionProbe.execution_health_source
            execution_probe_healthy = $executionProbe.execution_healthy
            positions = 0
            pending = 0
