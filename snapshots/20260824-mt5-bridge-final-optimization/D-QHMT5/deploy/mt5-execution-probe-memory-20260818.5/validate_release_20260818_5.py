#!/usr/bin/env python3
"""Pure-local schema, hash, baseline, and deployer check for MT5 .5."""
...
PREVIOUS = STAGE.parent / "mt5-trade-wake-priority-20260817.4-local"
MANIFEST = STAGE / "release-manifest.json"
FILES = {
    "__init__.py": STAGE / "app" / "__init__.py",
...
def main():
    ...
    require(before["build_id"] == previous["release"] ==
            "mt5-trade-wake-priority-20260817.4",
            "production-before release mismatch")
    require(before["app_init_present"] is False,
            "production-before package marker mismatch")
    ...
    markers = (
        ...
        'Restore-BackupFiles',
        'production app package drift: expected __init__.py to be absent',
        'Copy-Item -LiteralPath (Join-Path $Stage "app\\__init__.py")',
        '"/mt5/connection/status?execution_probe=true"',
        '$executionProbe.execution_health_source -ne "memory"',
    ...
    require("execution_probe_health()" in main_source,
            "memory execution health call missing")
    probe_source = main_source.split(
        "async def connection_status(execution_probe: bool = False):", 1,
    )[1].split("# ", 1)[0]
    require("_EXECUTION.metrics()" not in probe_source,
            "execution probe still calls full metrics")
    health_source = runtime_source.split(
        "def execution_probe_health(self):", 1,
    )[1].split("def metrics(self):", 1)[0]
    require("self._connection()" not in health_source,
            "memory execution health accesses the durable database")
    require("native_call" not in health_source,
            "memory execution health performs native RPC")
    require('"source": "memory"' in health_source,
            "memory execution health source marker missing")
