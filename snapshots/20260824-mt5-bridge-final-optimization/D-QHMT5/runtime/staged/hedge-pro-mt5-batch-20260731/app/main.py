...
_MT5_API_EXECUTOR = ThreadPoolExecutor(max_workers=1, thread_name_prefix="mt5-api")


def _mt5_call(func, *args, **kwargs):
    return _MT5_API_EXECUTOR.submit(func, *args, **kwargs).result()
...
def _refresh_deal_price(request):
...
def _execute_order(payload):
    ...
    result, retries = _send_with_requote(request)
    if result is None:
        return _unknown(
            request_id,
...
def _execute_cancel_all(payload):
    ...
    unknown = False
    for order in orders:
        result = _mt5_call(mt5.order_send, {"action": mt5.TRADE_ACTION_REMOVE, "order": order.ticket})
        if result is None:
            failed += 1
            unknown = True
        elif result.retcode == getattr(mt5, "TRADE_RETCODE_DONE", 10009):
