...
BRIDGE_BUILD_ID = os.getenv(
    "MT5_BRIDGE_BUILD_ID", "mt5-admission-snapshot-capability-20260821.86"
)
...
# Queue preflight may reuse only a broker-published, non-invalidated snapshot
# inside this narrow budget. It must not issue native MT5 I/O while a trade is
# waiting on the process-isolated owner.
POSITIONS_ADMISSION_MAX_AGE_MS = max(
    0,
    min(
        POSITIONS_STALE_MAX_MS,
        int(os.getenv("POSITIONS_ADMISSION_MAX_AGE_MS", "200")),
    ),
)
# Keep the broker snapshot warm only while the account is idle.  The warmer
# never runs during an admitted trade or the short post-trade quiet window,
# and PositionSnapshotCache coalesces any overlapping refresh into one native
# read.  This is deliberately bounded so it improves admission hit rate
# without turning the terminal read lane into a polling queue.
POSITION_ADMISSION_WARMER_SEC = max(
    0.25, min(5.0, float(os.getenv("POSITION_ADMISSION_WARMER_SEC", "0.50")))
)
...
# A native read which overruns this budget is treated as a terminal-side
# stall.  The MetaTrader5 extension cannot be cancelled safely once started;
# short-circuit subsequent ordinary reads so they cannot queue ahead of a
# trade, while the versioned position/tick caches continue serving data.
SLOW_NATIVE_READ_THRESHOLD_MS = max(
    50, min(5000, int(os.getenv("MT5_SLOW_NATIVE_READ_THRESHOLD_MS", "300")))
)
SLOW_NATIVE_READ_COOLDOWN_MS = max(
    100, min(10000, int(os.getenv("MT5_SLOW_NATIVE_READ_COOLDOWN_MS", "1200")))
)
...
_SLOW_NATIVE_READ_LOCK = threading.RLock()
_SLOW_NATIVE_READ_OPEN_UNTIL = {}
_SLOW_NATIVE_READ_STATS = {}
_POSITION_ADMISSION_STATS_LOCK = threading.Lock()
_POSITION_ADMISSION_STATS = {
    "hits": 0,
    "misses": 0,
    "warm_refreshes": 0,
    "warm_skips_trade": 0,
...
_POSITION_ADMISSION_WARMER_TASK = None
...
def _slow_native_read_open(method):
    key = str(method or "")
    now = time.monotonic()
    with _SLOW_NATIVE_READ_LOCK:
        until = float(_SLOW_NATIVE_READ_OPEN_UNTIL.get(key) or 0.0)
        if until <= now:
            _SLOW_NATIVE_READ_OPEN_UNTIL.pop(key, None)
            return False
        return True


def _record_slow_native_read(method, priority, timing):
    name = str(method or "")
    if priority <= MT5_API_PRIORITY_TRADE or name not in {
        "positions_get", "symbol_info_tick", "history_deals_get",
    }:
        return
    try:
        run_ms = float((timing or {}).get("run_ms") or 0.0)
    except (TypeError, ValueError):
        run_ms = 0.0
    with _SLOW_NATIVE_READ_LOCK:
        stats = _SLOW_NATIVE_READ_STATS.setdefault(name, {
            "slow_count": 0, "last_run_ms": 0.0,
            "last_slow_at_ms": 0, "open_until_ms": 0,
        })
        stats["last_run_ms"] = round(run_ms, 3)
        if run_ms < SLOW_NATIVE_READ_THRESHOLD_MS:
            return
        now = time.monotonic()
        _SLOW_NATIVE_READ_OPEN_UNTIL[name] = (
            now + SLOW_NATIVE_READ_COOLDOWN_MS / 1000.0
        )
        stats["slow_count"] = int(stats.get("slow_count") or 0) + 1
        stats["last_slow_at_ms"] = int(time.time_ns() // 1_000_000)
        stats["open_until_ms"] = int(
            time.time_ns() // 1_000_000 + SLOW_NATIVE_READ_COOLDOWN_MS
        )


def _slow_native_read_metrics():
    with _SLOW_NATIVE_READ_LOCK:
        now = time.monotonic()
        return {
            "threshold_ms": SLOW_NATIVE_READ_THRESHOLD_MS,
            "cooldown_ms": SLOW_NATIVE_READ_COOLDOWN_MS,
            "open": {
                name: max(0, round((until - now) * 1000, 3))
                for name, until in _SLOW_NATIVE_READ_OPEN_UNTIL.items()
                if until > now
            },
            "stats": {name: dict(values)
                      for name, values in _SLOW_NATIVE_READ_STATS.items()},
        }
...
def _mt5_call(func, *args, _priority=None, _metric_name=None,
    ...
    try:
        ...
        _record_slow_native_read(
            getattr(func, "__name__", ""), priority, timing,
        )
...
def _try_control_native_read(func, *args, _priority=MT5_API_PRIORITY_NORMAL,
    ...
    quiet = max(
        float(_quiet_seconds or 0.0),
        POSITIONS_TRADE_QUIET_MS / 1000.0,
    )
    if _MT5_TRADE_GATE.trade_activity(quiet):
        return _CONTROL_READ_DEFERRED
    if _slow_native_read_open(method):
        return _CONTROL_READ_DEFERRED
    ...
    return _mt5_call(
        ...
        _read_gate_quiet_seconds=quiet,
        **kwargs
    )
...
async def _position_admission_refresh_loop():
    """Keep a small broker-fresh admission window warm while idle."""
    while True:
        await asyncio.sleep(POSITION_ADMISSION_WARMER_SEC)
        if not mgr.connected:
            continue
        if _MT5_TRADE_GATE.trade_activity(
                POSITIONS_TRADE_QUIET_MS / 1000.0):
            _position_admission_metric("warm_skips_trade")
            continue
        try:
            if _POSITION_CACHE.get_admission(POSITIONS_ADMISSION_MAX_AGE_MS):
                continue
            await _control_call_async(_POSITION_CACHE.get)
            _position_admission_metric("warm_refreshes")
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            _position_admission_metric("warm_errors")
            logger.debug("idle admission snapshot refresh deferred: %s", exc)
...
async def on_startup():
    global _ACCOUNT_SNAPSHOT_REFRESH_TASK, _POSITION_ADMISSION_WARMER_TASK
    ...
    _POSITION_ADMISSION_WARMER_TASK = asyncio.create_task(
        _position_admission_refresh_loop()
    )

@app.on_event("shutdown")
async def on_shutdown():
    global _ACCOUNT_SNAPSHOT_REFRESH_TASK, _POSITION_ADMISSION_WARMER_TASK
    warmer_task = _POSITION_ADMISSION_WARMER_TASK
    _POSITION_ADMISSION_WARMER_TASK = None
    if warmer_task is not None:
        warmer_task.cancel()
        await asyncio.wait({warmer_task}, timeout=SHUTDOWN_STEP_SEC)
...
def _fetch_bounded_positions():
    if (_MT5_TRADE_GATE.trade_activity(
            POSITIONS_TRADE_QUIET_MS / 1000.0)
            or _slow_native_read_open("positions_get")):
        return _CONTROL_READ_DEFERRED
    value = _mt5_call(
        mt5.positions_get,
...
def _position_admission_metric(name):
    with _POSITION_ADMISSION_STATS_LOCK:
        _POSITION_ADMISSION_STATS[name] = int(
            _POSITION_ADMISSION_STATS.get(name, 0)
        ) + 1


def _position_admission_stats():
    with _POSITION_ADMISSION_STATS_LOCK:
        return dict(_POSITION_ADMISSION_STATS)
...
async def health(deep: bool = False):
    ...
    return {
        ...
        "slow_native_read_circuit": _slow_native_read_metrics(),
        "position_admission": _position_admission_stats(),
        "deep": bool(deep),
    }
...
async def terminal_state_ws(websocket: WebSocket):
    """Push versioned terminal/position state without polling native MT5."""
    ...
    try:
        while True:
            ...
            payload = {
                "type": "terminal_state",
                "instance": INSTANCE_NAME,
                "connected": bool(identity.get("verified")),
                "account": identity.get("actual_login"),
                "positions": (snapshot or {}).get("positions", []),
                "snapshot_boot": (snapshot or {}).get("snapshot_boot"),
                "snapshot_seq": (snapshot or {}).get("snapshot_seq"),
                "snapshot_ts_ms": (snapshot or {}).get("snapshot_ts_ms"),
                "snapshot_stale": (snapshot or {}).get("snapshot_stale", True),
...
async def get_positions(symbol: Optional[str] = None, authoritative: bool = False,
    ...
    if authoritative and admission:
        raise HTTPException(
            400,
            "authoritative and admission position snapshot modes are exclusive",
        )
    quiet_seconds = POSITIONS_TRADE_QUIET_MS / 1000.0
    if admission:
        # This endpoint branch must remain native-I/O free. In particular, do
        # not call `_ensure_mt5_async`, `_mt5_call`, or `mgr.ping` here: its
        # sole purpose is to keep pre-dispatch proof reads off the native order
        # owner when a just-published broker snapshot is available.
        snapshot = _POSITION_CACHE.get_admission(
            POSITIONS_ADMISSION_MAX_AGE_MS
        )
        if snapshot is None:
            _position_admission_metric("misses")
            raise HTTPException(
                503,
                "MT5 admission positions snapshot unavailable",
            )
        _position_admission_metric("hits")
    elif authoritative:
        await _ensure_mt5_async()
        try:
    ...
    snapshot["snapshot_authoritative"] = bool(authoritative or admission)
    snapshot["snapshot_admission"] = bool(admission)
    if admission:
        snapshot["snapshot_admission_max_age_ms"] = (
            POSITIONS_ADMISSION_MAX_AGE_MS
        )
