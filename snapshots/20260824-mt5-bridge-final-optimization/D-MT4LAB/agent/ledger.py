...
class Ledger:
    ...
    def close(self):
        """Close the per-agent SQLite handle after the ASGI lifespan ends.

        TestClient creates and tears down an event loop for every lifespan.  A
        module-level Ledger otherwise keeps its WAL/SHM handles alive across
        those short-lived app instances (especially visible on Windows),
        which can delay the next actor's first terminal result or prevent temp
        databases from being removed.  Keep this idempotent so shutdown and
        defensive cleanup can safely call it more than once.
        """
        with self._lock:
            if self._closed:
                return
    ...
    def reopen(self):
        """Re-open a handle when an app object is reused for another lifespan."""
        with self._lock:
            if not self._closed:
                return
