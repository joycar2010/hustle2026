...
SYNC_WAIT_SEC = float(os.getenv("SYNC_WAIT_SEC", "0.82"))
...
def _pending_response(cmd_id: str, state: str = "DISPATCHING"):
...
async def _start_execution_actor():
    global _execution_queue, _execution_actor_task
...
async def _submit_execution(cmd_id: str, op: str, payload: dict):
    ...
    try:
        ok, response = await asyncio.wait_for(asyncio.shield(future), timeout=SYNC_WAIT_SEC)
...
async def place_order(req: OrderRequest):
    ...
    payload["otoken"] = _otoken(cmd_id)
    return await _submit_execution(cmd_id, "order", payload)
...
async def close_all(req: CloseAllRequest):
    ...
    res = await asyncio.to_thread(_submit_and_wait, cmd_id, {"op": "close_all", "symbol": req.symbol})
    if res is None:
...
async def cancel_all(req: CancelAllRequest):
    _require_connected()
    cmd_id = uuid.uuid4().hex
    res = await asyncio.to_thread(_submit_and_wait, cmd_id, {"op": "cancel_all", "symbol": req.symbol})
    if res is None:
