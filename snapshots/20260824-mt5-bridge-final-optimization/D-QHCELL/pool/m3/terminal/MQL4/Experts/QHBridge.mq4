...
input int    TimerMs      = 10;           // command pickup cadence (runtime bounded to 10-15ms)
input int    CommandBurstMax = 8;         // drain commands already staged by the single Agent actor
input int    CommandHandoffGraceMs = 50;  // bridge handoff grace after a claimed command
input int    CommandHandoffPollMs = 2;    // bounded poll cadence inside the handoff grace
...
input int    RequoteRetries  = 1;          // Bounded retry count for explicit not-filled transient trade errors
...
int OnInit()
{
   ...
   // Existing charts may retain an older 100ms input. Keep the effective command
   // wake-up bounded even before saved inputs are refreshed during rollout.
   EventSetMillisecondTimer(MathMax(10, MathMin(TimerMs, 15)));
   LoadWatch();
   ExportMeta();
   ExportAccountFast();
   ExportPositions();
...
void OnTimer()
{
   ...
   int traded_count = ProcessCommandBurst();
   bool traded = traded_count > 0;
...
void OnTick()
{
   ...
   // second wake-up path. Publish positions here as well so a tick-handled trade
   // cannot leave the Agent reading the pre-trade snapshot until OnTimer runs.
   ulong command_start_us = GetMicrosecondCount();
   int traded_count = ProcessCommandBurst();
   TrackMax(g_perf_command_ms, ElapsedMs(command_start_us));
   if(traded_count <= 0) return;

   g_history_dirty = true;
   ulong positions_start_us = GetMicrosecondCount();
   ExportPositions();
   TrackMax(g_perf_positions_ms, ElapsedMs(positions_start_us));
   g_last_position_ms = GetTickCount();
}
...
{
   g_position_seq++;
   long snapshot_ts = (long)TimeLocal();
   long snapshot_ts_ms = snapshot_ts * 1000 +
                         (long)((GetMicrosecondCount() / 1000) % 1000);
   // Keep the published timestamp monotonic even when the local second rolls
   // over between two timer callbacks.
   if(snapshot_ts_ms <= g_snapshot_ts_ms) snapshot_ts_ms = g_snapshot_ts_ms + 1;
   g_snapshot_ts_ms = snapshot_ts_ms;
   string arr = "";
   int n = 0;
   for(int i = 0; i < OrdersTotal(); i++)
   {
   ...
   j += ",\"snapshot_ts\":" + IntegerToString(snapshot_ts_ms / 1000);
   j += ",\"snapshot_ts_ms\":" + IntegerToString(snapshot_ts_ms);
   j += ",\"ts\":" + IntegerToString(snapshot_ts_ms / 1000) + "}";
...
int ProcessCommandBurst()
{
   int limit = MathMax(1, MathMin(CommandBurstMax, 8));
   uint grace_ms = (uint)MathMax(0, MathMin(CommandHandoffGraceMs, 250));
   int poll_ms = MathMax(1, MathMin(CommandHandoffPollMs, 2));
   int handled = 0;
   uint last_handled_ms = 0;
   while(handled < limit)
   {
      if(ProcessOneCommand())
      {
         handled++;
         last_handled_ms = GetTickCount();
         continue;
      }

      // The Agent exposes command N+1 only after the dispatch tombstone for N
      // is durable. Keep this event slice alive briefly so a command published
      // on that handoff cannot fall through to a later MT4 timer/tick cycle.
      // An idle event never waits, and the post-command wait is strictly bounded.
      if(handled <= 0 || grace_ms == 0 ||
         GetTickCount() - last_handled_ms >= grace_ms) break;
      Sleep(poll_ms);
   }
   return handled;
}
...
void HandleCommand(string fname)
{
   ...
   {
      if(!WriteDispatchTombstone(cmd_id, op, otoken, observed_us))
         result = "{\"ok\":false,\"dispatch_durable\":false,\"error\":\"dispatch tombstone is not durable; broker call refused\"}";
      else if(op == "order")      result = ExecOrder(j, otoken, observed_us);
   ...
   // Publish broker truth before exposing the terminal result. This prevents a
   // fast result/status poll from observing the pre-trade positions snapshot,
   // and keeps early commands in a burst from waiting for later broker calls.
   if(op == "order" || op == "close" || op == "close_all")
   {
      ulong positions_start_us = GetMicrosecondCount();
      ExportPositions();
      TrackMax(g_perf_positions_ms, ElapsedMs(positions_start_us));
      g_last_position_ms = GetTickCount();
      result = WithPositionSnapshotTrace(result);
   }
...
bool WriteDispatchTombstone(string cmd_id, string op, string otoken, ulong observed_us)
{
   ...
   uint written = FileWriteString(h, j);
   FileFlush(h);
   FileClose(h);
   if(written != (uint)StringLen(j)) { FileDelete(tmp); return false; }
   FileDelete(rel);
   if(!FileMove(tmp, 0, rel, FILE_REWRITE)) { FileDelete(tmp); return false; }
   return DispatchExists(cmd_id);
}
...
string WithPositionSnapshotTrace(string result)
{
   if(StringLen(result) < 1) result = "{\"ok\":false,\"error\":\"empty result\"}";
   if(StringGetCharacter(result, StringLen(result)-1) == '}')
      result = StringSubstr(result, 0, StringLen(result)-1);
   return result + ",\"position_snapshot_boot\":" + IntegerToString(g_snapshot_boot) +
          ",\"position_snapshot_seq\":" + IntegerToString(g_position_seq) + "}";
}
   ...
   if(op == "order")
   {
      ...
      if(existing >= 0 && OrderSelect(existing, SELECT_BY_TICKET))
         return WithTrace("{\"ok\":true,\"op\":\"order\",\"ticket\":" + IntegerToString(existing) +
                ...
                ",\"retcode\":10009,\"certainty\":\"FILLED\",\"partial\":false,\"reconciled\":true}", observed_us, 0, GetMicrosecondCount());
   }
   else if(op == "close")
   {
      ...
      if(tk > 0 && (!OrderSelect(tk, SELECT_BY_TICKET) || OrderCloseTime() != 0))
         return WithTrace("{\"ok\":true,\"op\":\"close\",\"ticket\":" + IntegerToString(tk) +
                ",\"reconciled\":true,\"retcode\":10009,\"certainty\":\"EXECUTED\"}", observed_us, 0, GetMicrosecondCount());
   }
   return WithTrace("{\"ok\":false,\"unknown\":true,\"certainty\":\"UNKNOWN\",\"op\":" + JStr(op) +
...
bool IsExplicitNotFilledTradeError(int err)
{
   return (err == 129 || err == 130 || err == 131 || err == 132 ||
           err == 133 || err == 134 || err == 135 || err == 136 ||
           err == 138 || err == 140 || err == 141 || err == 145 ||
           err == 146 || err == 147 || err == 148 || err == 149 ||
           err == 150 || err == 4106 || err == 4107 || err == 4108 ||
           err == 4109);
}

string TradeFailureCertaintyJson(int err)
{
   if(IsExplicitNotFilledTradeError(err))
      return ",\"not_filled\":true,\"certainty\":\"NOT_FILLED\"";
   // Trade timeout 128 and every unclassified post-dispatch error stay
   // ambiguous. The Agent must freeze publication until broker truth resolves.
   return ",\"unknown\":true,\"certainty\":\"UNKNOWN\"";
}

bool IsRetryableTradeError(int err)
{
   // These MT4 errors prove that no order was accepted. Do not add timeout
   // errors such as 128: their broker outcome is ambiguous and redispatching
   // could duplicate an order.
   return (err == 135 || err == 136 || err == 138);
}
...
string ExecOrder(string j, string otoken, ulong observed_us)
{
   ...
   int    max_requotes = MathMax(0, MathMin(RequoteRetries, 1));
   ...
   while(true)
   {
      ...
      if(ticket >= 0) break;
      err = GetLastError();
      if(original_err == 0) original_err = err;
      if(!IsRetryableTradeError(err) || requote_retries >= max_requotes) break;
      requote_retries++;
      if(RequoteRetryMs > 0) Sleep(MathMin(RequoteRetryMs, 25));
   }
   ulong broker_end_us = GetMicrosecondCount();
   if(ticket < 0)
   {
      return WithTrace("{\"ok\":false,\"op\":\"order\",\"retcode\":" + IntegerToString(err) +
             ",\"original_retcode\":" + IntegerToString(original_err) +
             ",\"final_retcode\":" + IntegerToString(err) +
             ",\"retry_count\":" + IntegerToString(requote_retries) +
             ",\"requote_retries\":" + IntegerToString(requote_retries) +
             ",\"error\":\"OrderSend err " + IntegerToString(err) + "\"" +
             TradeFailureCertaintyJson(err) + "}", observed_us, broker_start_us, broker_end_us);
   }
   ...
   return WithTrace("{\"ok\":true,\"op\":\"order\",\"ticket\":" + IntegerToString(ticket) +
           ...
           ",\"original_retcode\":" + IntegerToString(original_err) +
           ",\"final_retcode\":10009,\"retry_count\":" + IntegerToString(requote_retries) +
           ",\"requote_retries\":" + IntegerToString(requote_retries) +
           ",\"retcode\":10009,\"certainty\":\"FILLED\",\"partial\":false,\"reconciled\":false}", observed_us, broker_start_us, broker_end_us);
}
...
string ExecClose(string j, ulong observed_us)
{
   ...
   if(tk > 0)
   {
      if(!OrderSelect(tk, SELECT_BY_TICKET))
         return WithTrace("{\"ok\":true,\"op\":\"close\",\"ticket\":" + IntegerToString(tk) + ",\"reconciled\":true,\"note\":\"not found=closed\",\"retcode\":10009,\"certainty\":\"EXECUTED\"}", observed_us, 0, GetMicrosecondCount());
      if(OrderCloseTime() != 0)
         return WithTrace("{\"ok\":true,\"op\":\"close\",\"ticket\":" + IntegerToString(tk) + ",\"reconciled\":true,\"note\":\"already closed\",\"retcode\":10009,\"certainty\":\"EXECUTED\"}", observed_us, 0, GetMicrosecondCount());
      ...
      ulong broker_end_us = GetMicrosecondCount();
      if(closed)
         return WithTrace("{\"ok\":true,\"op\":\"close\",\"ticket\":" + IntegerToString(tk) + ",\"price\":" + DoubleToString(cprice,5) + ",\"volume\":" + DoubleToString(lots,2) + ",\"original_retcode\":" + IntegerToString(original_err) + ",\"final_retcode\":10009,\"retry_count\":" + IntegerToString(requote_retries) + ",\"requote_retries\":" + IntegerToString(requote_retries) + ",\"retcode\":10009,\"certainty\":\"EXECUTED\"}", observed_us, broker_start_us, broker_end_us);
      return WithTrace("{\"ok\":false,\"op\":\"close\",\"retcode\":" + IntegerToString(err) + ",\"original_retcode\":" + IntegerToString(original_err) + ",\"final_retcode\":" + IntegerToString(err) + ",\"retry_count\":" + IntegerToString(requote_retries) + ",\"requote_retries\":" + IntegerToString(requote_retries) + TradeFailureCertaintyJson(err) + "}", observed_us, broker_start_us, broker_end_us);
   }
   return WithTrace("{\"ok\":false,\"op\":\"close\",\"error\":\"EXACT_TICKET_REQUIRED\"}", observed_us, 0, GetMicrosecondCount());
}
...
string ExecCloseAll(string j)
{
   ...
   int closed = 0, failed = 0, ambiguous = 0, total_requote_retries = 0; string arr = "";
   for(int i = OrdersTotal()-1; i >= 0; i--)
   {
      ...
      double cprice = 0.0; int err = 0; int original_err = 0; int requote_retries = 0;
      int max_requotes = MathMax(0, MathMin(RequoteRetries, 1)); bool did_close = false;
      while(true)
      {
         RefreshRates();
      ...
      if(arr != "") arr += ",";
      if(did_close) { closed++; arr += "{\"ticket\":" + IntegerToString(tkt) + ",\"success\":true,\"order\":" + IntegerToString(tkt) + ",\"original_retcode\":" + IntegerToString(original_err) + ",\"final_retcode\":10009,\"retry_count\":" + IntegerToString(requote_retries) + ",\"requote_retries\":" + IntegerToString(requote_retries) + "}"; }
      else { failed++; if(!IsExplicitNotFilledTradeError(err)) ambiguous++; arr += "{\"ticket\":" + IntegerToString(tkt) + ",\"success\":false,\"retcode\":" + IntegerToString(err) + ",\"original_retcode\":" + IntegerToString(original_err) + ",\"final_retcode\":" + IntegerToString(err) + ",\"retry_count\":" + IntegerToString(requote_retries) + ",\"requote_retries\":" + IntegerToString(requote_retries) + TradeFailureCertaintyJson(err) + "}"; }
   }
   if(ambiguous > 0)
      return "{\"ok\":false,\"unknown\":true,\"certainty\":\"UNKNOWN\",\"op\":\"close_all\",\"closed\":" + IntegerToString(closed) + ",\"failed\":" + IntegerToString(failed) + ",\"requote_retries\":" + IntegerToString(total_requote_retries) + ",\"results\":[" + arr + "]}";
