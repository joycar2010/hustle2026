...
# Keep the Cell health gate aligned with the bridge release being supervised.
# Cell is launched by the independent QHCELL-Agent scheduled task and does
# not inherit each bridge instance's .env, so the release gate is explicit.
# The deployment rollback restores this file together with the bridge code.
...
def spawn_mt5(s):
    ...
    p = subprocess.Popen([VENV_PY, "-m", "uvicorn", "app.main:app",
                         ...
                         # MT5 IPC is latency-sensitive, but must remain at
                         # Normal rather than High/Realtime for host safety.
                         # The native owner and terminal inherit this class.
