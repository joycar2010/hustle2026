...
def get_db_connection():
    ...
    try:
        conn = psycopg2.connect(
            ...
            password=database_config.get("password", "<REDACTED_PASSWORD>")
        )
        return conn
    except Exception as e:
...
def load_instances() -> Dict:
    """Load host-scoped instances from file or the legacy database source."""
    if config.get("instance_source", "database").lower() == "file":
        return load_instances_from_file()
            ...
            try:
                if proc.info['name'] == 'terminal64.exe' and proc.info['exe']:
                    ...
                    if proc_path == target_path:
                        # 勿记 cmdline: 终端以 /password: 明文参数启动, 曾把密码刷进日志(1.3GB/90min)
                        logger.debug(f"Instance running: path={target_path}, account={account}")
                        return True
        ...
        for proc in psutil.process_iter(['name', 'exe', 'pid', 'cmdline']):
            try:
                if proc.info['name'] == 'terminal64.exe' and proc.info['exe']:
                    proc_path = os.path.normpath(proc.info['exe']).lower()
                    if proc_path == target_path:
                        logger.debug(f"Found process: PID={proc.info['pid']}, path={target_path}, account={account}")
            ...
            for proc in psutil.process_iter(['pid', 'name', 'exe', 'cmdline']):
                try:
                    if proc.info['name'] == 'terminal64.exe' and proc.info['exe']:
                        proc_path = os.path.normpath(proc.info['exe']).lower()
                        if proc_path == target_path:
                            logger.info(f"Found matching process: PID={proc.info['pid']}")
...
def _load_persisted_bridge_ports() -> Dict[str, int]:
    ...
    """
    configured = config.get("bridge_service_ports")
...
def start_instance(instance_name: str, wait_seconds: int = 5):
    ...
    cfg = instances[instance_name]
    bridge_service = cfg.get("bridge_service")
    if bridge_service:
        try:
            bridge_result = start_bridge(bridge_service)
        except HTTPException as exc:
            if exc.status_code != 500:
                raise
            bridge_result = {"success": True, "message": "Bridge startup in progress"}
...
# ====================== Bridge 实例控制 ======================
def _resolve_bridge_dir(service_name: str) -> Path:
    """Resolve both legacy D:\\<service> and QHMT5 instance layouts."""
    candidates = [
        Path(f"D:/{service_name}"),
        Path(f"D:/QHMT5/instances/{service_name}"),
    ]
    short_name = service_name.removeprefix("hustle-mt5-mt5-")
    candidates.append(Path(f"D:/QHMT5/instances/{short_name}"))
...
def _supervisor_hold_file(service_name: str, deploy_dir: Path) -> Optional[Path]:
    root = config.get("supervisor_root")
    if not root and "qhmt5" in str(deploy_dir).lower():
        root = "D:/QHMT5"
    if not root:
        return None
    hold_dir = Path(root) / "holds"
    hold_dir.mkdir(parents=True, exist_ok=True)
    short_name = service_name.removeprefix("hustle-mt5-mt5-")
    return hold_dir / f"{short_name}.hold"


def _read_bridge_env(deploy_dir: Path) -> Dict[str, str]:
    values: Dict[str, str] = {}
    with open(deploy_dir / ".env", encoding="utf-8-sig") as f:
        for raw_line in f:
            line = raw_line.strip()
...
def get_bridge_status(service_name: str):
    ...
    try:
        import socket as _socket
        deploy_dir = _resolve_bridge_dir(service_name)
...
def start_bridge(service_name: str):
    ...
    port = None
    try:
        env_values = _read_bridge_env(deploy_dir)
        env.update(env_values)
        port = int(env_values.get("SERVICE_PORT", "0"))
    except Exception as e:
    ...
    hold_file = _supervisor_hold_file(service_name, deploy_dir)
    was_held = bool(hold_file and hold_file.exists())
    if was_held:
        hold_file.unlink()

    is_qh_layout = "qhmt5" in str(deploy_dir).lower()
    if is_qh_layout and was_held:
        return {"service_name": service_name, "operation": "start",
                "success": True, "message": "Supervisor resume requested"}
    if is_qh_layout:
        family = "bybit" if "bybit" in env.get("MT5_SERVER", "").lower() else "ic"
        python_exe = Path(f"D:/QHMT5/runtime/releases/v1/venv-{family}/Scripts/python.exe")
        app_dir = deploy_dir
    ...
    try:
        ...
        proc = subprocess.Popen(
            [str(python_exe), "-m", "uvicorn", app_module,
...
def stop_bridge(service_name: str):
    ...
    hold_file = _supervisor_hold_file(service_name, deploy_dir)
    if hold_file:
        hold_file.write_text(f"manual stop {datetime.utcnow().isoformat()}Z\n", encoding="ascii")
...
# ====================== 启动进度状态机 (2026-07-09) ======================
# 观测每个桥的开机相位, 补足 /health 只有终态(mt5:true/false)、看不到启动过程的缺口。
# 相位: stopped(端口未起) → starting(端口起但/health未答) → attaching(桥起但MT5未挂载)
#      → healthy(mt5:true)。attaching 超时=卡住(授权失败/IPC僵死/登录未完成), 显式亮红。
_BOOT_TRACK = {}
_BOOT_STUCK_S = 120  # attaching 超过此秒数判定卡住
_BOOT_LAST_REFRESH = 0.0
_BOOT_TASK = None  # 强引用必须留着: asyncio 只弱引用 task, 被 GC 即无声死亡(0709 实测冻结30min后)


async def _probe_bridge_health(port):
    if not port:
        return False, False
    try:
        async with httpx.AsyncClient(timeout=2.0) as c:
            ...
            if r.status_code == 200:
                return True, bool((r.json() or {}).get("mt5"))
    except Exception:
        pass
    return False, False


async def _refresh_boot_track():
    global _BOOT_LAST_REFRESH
    now = time.time()
    try:
        bridges = _query_bridge_services_status()
    except Exception:
    ...
    for b in bridges:
        svc = b["service"]
        port = BRIDGE_SERVICE_PORTS.get(svc)
        tcp_alive = b["state"] in ("running", "paused")
        reachable, mt5_attached = (False, False)
        if tcp_alive:
            reachable, mt5_attached = await _probe_bridge_health(port)
        if not tcp_alive:
            phase = "stopped"
        elif not reachable:
            phase = "starting"
        elif not mt5_attached:
            phase = "attaching"
        else:
            phase = "healthy"
        prev = _BOOT_TRACK.get(svc)
        since = prev["since"] if (prev and prev.get("phase") == phase) else now
        elapsed = int(now - since)
        _BOOT_TRACK[svc] = {
            "service": svc, "port": port, "pid": b.get("pid"),
            "phase": phase, "since": since, "elapsed": elapsed,
            "mt5": mt5_attached, "reachable": reachable,
            "stuck": (phase == "attaching" and elapsed >= _BOOT_STUCK_S),
        }
    _BOOT_LAST_REFRESH = time.time()


async def _boot_track_loop():
    await asyncio.sleep(3)
    while True:
        try:
            await _refresh_boot_track()
        except Exception as e:
            logger.debug(f"[BootTrack] refresh failed: {e}")
        await asyncio.sleep(5)


def _boot_summary():
    items = list(_BOOT_TRACK.values())
    total = len(items) or len(BRIDGE_SERVICES)
    healthy = sum(1 for i in items if i["phase"] == "healthy")
    return {
        "timestamp": int(time.time()),
        "uptime_seconds": int(time.time() - AGENT_START_TS),
        "progress_pct": round(100.0 * healthy / total, 1) if total else 0,
        "summary": {
            "total": total, "healthy": healthy,
            "attaching": sum(1 for i in items if i["phase"] == "attaching"),
            "starting": sum(1 for i in items if i["phase"] == "starting"),
            "stopped": sum(1 for i in items if i["phase"] == "stopped"),
            "stuck": sum(1 for i in items if i.get("stuck")),
        },
        "stuck_services": [i["service"] for i in items if i.get("stuck")],
        "bridges": sorted(items, key=lambda x: x.get("port") or 0),
    }


def _ensure_boot_task():
    """(重)拉起 boot 轨迹循环: 覆盖没起过/死亡(异常/GC/取消)/卡死(>60s没刷)三种情况。"""
    global _BOOT_TASK
    if _BOOT_TASK is not None and not _BOOT_TASK.done():
        if _BOOT_LAST_REFRESH and (time.time() - _BOOT_LAST_REFRESH) > 60:
            _BOOT_TASK.cancel()
            logger.warning("[BootTrack] loop wedged (no refresh >60s) — recycling")
        else:
            return
    elif _BOOT_TASK is not None:
        try:
            exc = _BOOT_TASK.exception()
            logger.error(f"[BootTrack] loop task died: {exc!r} — restarting")
        except asyncio.CancelledError:
            logger.error("[BootTrack] loop task was cancelled — restarting")
    _BOOT_TASK = asyncio.get_event_loop().create_task(_boot_track_loop())


@app.get("/boot-status")
async def boot_status():
    """开机进度快照(无需API key, 供 admin 面板/本地dashboard/健康检查轮询)。"""
    _ensure_boot_task()
    out = _boot_summary()
    out["refresh_age_s"] = int(time.time() - _BOOT_LAST_REFRESH) if _BOOT_LAST_REFRESH else None
    return out


_DASH_HTML = """<!doctype html><html><head><meta charset=utf-8>
<title>MT5 Agent 启动进度</title><meta name=viewport content="width=device-width,initial-scale=1">
<style>body{background:#0b0e14;color:#e5e7eb;font:13px/1.5 -apple-system,Segoe UI,sans-serif;margin:0;padding:16px}
h1{font-size:15px;margin:0 0 4px}.sub{color:#6b7280;font-size:11px;margin-bottom:12px}
.bar{height:8px;background:#1f2733;border-radius:4px;overflow:hidden;margin:8px 0 16px}
.bar>i{display:block;height:100%;background:#22c55e;transition:width .4s}
table{width:100%;border-collapse:collapse}td,th{padding:6px 8px;text-align:left;border-bottom:1px solid #1a2029}
th{color:#6b7280;font-weight:500;font-size:11px}.dot{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:6px}
.b{padding:2px 8px;border-radius:10px;font-size:11px;font-weight:600}
.healthy{background:#0d2b1a;color:#22c55e}.attaching{background:#2b230d;color:#f0b90b}
.starting{background:#0d1f2b;color:#38bdf8}.stopped{background:#2b0d0d;color:#ef4444}.stuck{background:#3b0d0d;color:#fca5a5;animation:pulse 1s infinite}
@keyframes pulse{50%{opacity:.5}}.mono{font-family:ui-monospace,monospace;color:#9ca3af}</style></head>
<body><h1>MT5 Agent 启动进度</h1><div class=sub id=sub>加载中...</div>
<div class=bar><i id=barfill style=width:0%></i></div>
<table><thead><tr><th>桥</th><th>端口</th><th>相位</th><th>MT5</th><th>停留</th><th>PID</th></tr></thead><tbody id=rows></tbody></table>
<script>
const C={healthy:'#22c55e',attaching:'#f0b90b',starting:'#38bdf8',stopped:'#ef4444'};
const L={healthy:'健康',attaching:'挂载中',starting:'启动中',stopped:'已停止'};
async function tick(){try{const d=await(await fetch('/boot-status')).json();
document.getElementById('barfill').style.width=d.progress_pct+'%';
const S=d.summary;document.getElementById('sub').textContent=
`进度 ${d.progress_pct}% · 健康 ${S.healthy}/${S.total}`+(S.stuck?` · ⚠卡住 ${S.stuck}`:'')+` · Agent运行 ${Math.floor(d.uptime_seconds/60)}分`;
document.getElementById('rows').innerHTML=d.bridges.map(b=>{const cls=b.stuck?'stuck':b.phase;
const lab=b.stuck?'卡住!':L[b.phase];const col=C[b.phase]||'#6b7280';
return `<tr><td><span class=dot style=background:${col}></span>${b.service.replace('hustle-mt5-','')}</td>
<td class=mono>:${b.port||'-'}</td><td><span class="b ${cls}">${lab}</span></td>
<td>${b.mt5?'<span style=color:#22c55e>✓已挂载</span>':'<span style=color:#6b7280>—</span>'}</td>
<td class=mono>${b.elapsed}s</td><td class=mono>${b.pid||'-'}</td></tr>`}).join('')}
catch(e){document.getElementById('sub').textContent='连接代理失败: '+e}}
...
def dashboard():
    """本地开机进度看板(RDP到服务器时浏览器直开 http://localhost:8765/dashboard)。"""
...
async def startup_event():
    ...
    # 启动进度状态机刷新(2026-07-09); _ensure_boot_task 持强引用+自愈
    _ensure_boot_task()

    # 先自动启动所有 MT5 客户端，再启动 Bridge 服务
    if config.get("auto_start_enabled", True):
        asyncio.create_task(_auto_start_mt5_then_bridges())
    else:
        logger.info("[AutoStartMT5] Disabled; supervisor owns process recovery")
