...
API_KEY       = os.getenv("API_KEY", "<REDACTED_API_KEY>")   # 与 mt5-bridge 同默认(生产用 go 共享 key)
...
SYNC_WAIT_SEC = max(0.05, float(os.getenv("SYNC_WAIT_SEC", "0.85")))
...
AGENT_BUILD_ID = "no123-agent-terminal-queue-20260805.4"
...
async def _submit_execution(cmd_id: str, op: str, payload: dict, ack_only: bool = False,
    ...
    if existing:
        ...
        async with _admission_lock:
            ...
            reserved = await asyncio.to_thread(ledger.reserve, cmd_id, op, intent_hash, payload)
            if not reserved:
                return _pending_response(cmd_id)
