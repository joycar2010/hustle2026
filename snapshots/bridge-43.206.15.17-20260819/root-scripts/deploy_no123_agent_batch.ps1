[CmdletBinding()]
param(
    [string]$ReleaseTag = 'batch-ladders-20260731'
)

$ErrorActionPreference = 'Stop'
$baseDir = 'D:\MT4LAB\agent'
$python = Join-Path $baseDir 'venv\Scripts\python.exe'
$stagedMain = Join-Path $baseDir 'main.py.batch.new'
$stagedLedger = Join-Path $baseDir 'ledger.py.batch.new'
$stagedBridge = Join-Path $baseDir 'filebridge.py.batch.new'
$backupDir = Join-Path $baseDir ("backups\{0}" -f $ReleaseTag)

foreach ($path in $stagedMain, $stagedLedger, $stagedBridge) {
    if (-not (Test-Path -LiteralPath $path)) { throw "Missing staged file: $path" }
}
& $python -m py_compile $stagedMain $stagedLedger $stagedBridge
...
Copy-Item -LiteralPath (Join-Path $baseDir 'ledger.py') -Destination $backupDir -Force
...
try {
    ...
    Copy-Item -LiteralPath $stagedLedger -Destination (Join-Path $baseDir 'ledger.py') -Force
    ...
    foreach ($instance in $instances) {
        ...
        if ([int]$health.execution_queue.max_pending_executions -ne 32) {
            throw "Agent port $($instance.Port) did not load execution capacity 32"
        }
    }
} catch {
    Copy-Item -LiteralPath (Join-Path $backupDir 'main.py') -Destination (Join-Path $baseDir 'main.py') -Force
    Copy-Item -LiteralPath (Join-Path $backupDir 'ledger.py') -Destination (Join-Path $baseDir 'ledger.py') -Force
...
[pscustomobject]@{
    ...
    LedgerHash = (Get-FileHash -LiteralPath (Join-Path $baseDir 'ledger.py') -Algorithm SHA256).Hash
