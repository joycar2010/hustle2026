...
def _isolated_worker_entry(db_path, executor_ref, stop_event, ready_event,
                           native_connection, worker_lock_path, parent_pid,
                           owner_id, worker_generation):
    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            if claimed is None:
                try:
                    # Pipe traffic wakes reads immediately. Durable trades do
                    # not need a shared Event: the WAL is polled every 10 ms,
                    # avoiding a Windows semaphore poisoned by a killed child.
                    native_ready = native_connection.poll(0.01)
                    native_request = native_connection.recv() if native_ready else None
...
class ProcessIsolatedExecutionCoordinator:
    ...
    def metrics(self):
        ...
        process_ready = bool(
            process_alive and worker_ready is not None and worker_ready.is_set()
        )
