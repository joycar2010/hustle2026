...
def _mt5_call(func, *args, _priority=None, **kwargs):
    ...
    is_background_position_read = (
        getattr(func, "__name__", "") == "positions_get"
        and priority > MT5_API_PRIORITY_TRADE
    )
    if is_background_position_read:
        _MT5_TRADE_GATE.begin_read(timeout=POSITIONS_READ_GATE_WAIT_MS / 1000.0)
    try:
...
def _run_post_trade_snapshot_refresh():
    ...
    if _MT5_TRADE_GATE.pending_trades() > 0:
        _schedule_post_trade_snapshot(10)
        return
    try:
        _POSITION_CACHE.get()
...
async def health():
    ta = None
    try:
        _ti = await _mt5_call_async(mt5.terminal_info)
...
async def connection_status():
    ...
    if mgr.connected:
        info = await _mt5_call_async(mt5.account_info)
        if info:
            mgr.ping()
            status.update({
...
async def reconnect():
    ok = await asyncio.to_thread(mgr.reconnect)
...
async def get_positions(symbol: Optional[str] = Query(None)):
    ...
    try:
        snapshot = await asyncio.to_thread(_POSITION_CACHE.get)
    except Exception as exc:
...
async def account_balance():
    ...
    if info is None:
        ...
        raise HTTPException(500, f"mt5.account_info() failed: {error}")
    mgr.ping()
    return {
        "balance":       info.balance,
        "equity":        info.equity,
...
async def account_info():
    ...
    total_swap = 0.0
    positions = await _mt5_call_async(mt5.positions_get) or []
    for p in positions:
        total_swap += p.swap
...
async def get_tick(symbol: str):
    await _ensure_mt5_async()
    tick = await _mt5_call_async(mt5.symbol_info_tick, symbol)
    if tick is None:
        error = await _mt5_call_async(mt5.last_error)
        raise HTTPException(404, f"No tick for symbol {symbol}: {error}")
    mgr.ping()
    return {
        "symbol": symbol,
        "bid":    tick.bid,
...
async def order_status(request_id: str):
    row = await asyncio.to_thread(_idem_get, request_id)
    if not row:
...
def _send_with_requote(request, sym_info=None):
    ...
    def _native_send():
        ...
        while True:
            if is_deal:
                _refresh_deal_price_native(request, sym_info)
            result = mt5.order_send(request=dict(request))
            if result is None:
                return None, retries
            if result.retcode not in _requote_codes() or retries >= REQUOTE_RETRIES or not is_deal:
                return result, retries
            if not _refresh_deal_price_native(request, sym_info):
                return result, retries
            retries += 1
            logger.warning(
    ...
    return _mt5_call(_native_send, _priority=MT5_API_PRIORITY_TRADE)
...
async def order_status_v3(request_id: str):
    status = await asyncio.to_thread(_EXECUTION.status, request_id)
    if status is None:
        raise HTTPException(404, "unknown request_id")
    return status
