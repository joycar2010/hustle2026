...
# During this window /mt5/positions serves bounded cached truth and cannot
# insert a native positions_get ahead of the next QH command preflight.
POSITIONS_TRADE_QUIET_MS = max(
    0,
    min(
        POSITIONS_STALE_MAX_MS,
        int(os.getenv("POSITIONS_TRADE_QUIET_MS", "350")),
    ),
)
...
def _refresh_account_snapshot_sync(include_history=False, wait_for_lock=False):
    ...
    read_token = None
    try:
        _MT5_TRADE_GATE.begin_read(timeout=0.0)
        read_reserved = True
        ...
        # A trade admitted after the short account read takes precedence over
        # positions/history enrichment. The last complete snapshot remains
        # usable within its explicit bounded-stale window.
        if not include_history or _MT5_TRADE_GATE.pending_trades() > 0:
    ...
    while True:
        ...
        try:
            await _control_call_async(
                _refresh_account_snapshot_sync, _history_refresh_due(), False,
            )
...
async def on_shutdown():
    ...
    if refresh_task is not None:
        ...
        try:
            ...
            pass
        await _control_call_async(_wait_for_account_snapshot_refresh)
    ...
    if execution is not None:
        await _control_call_async(execution.shutdown)
    mgr.disconnect()
    _MT5_API_EXECUTOR.shutdown(wait=True, cancel_futures=False)
    _ORDER_STATUS_EXECUTOR.shutdown(wait=True, cancel_futures=False)
    _CONTROL_EXECUTOR.shutdown(wait=True, cancel_futures=False)
...
async def order_status_v3(request_id: str):
    ...
    status = await loop.run_in_executor(
        _ORDER_STATUS_EXECUTOR, _EXECUTION.status, request_id
    )
    if status is None:
        raise HTTPException(404, "unknown request_id")
    return status
