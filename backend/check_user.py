...
async def check():
    async with AsyncSessionLocal() as db:
        r = await db.execute(text("SELECT id FROM users WHERE username='cq456'"))
        row = r.fetchone()
        if not row:
        ...
        r2 = await db.execute(text("SELECT id, platform_id, account_name FROM accounts WHERE user_id = :uid"), {"uid": uid})
        accs = r2.fetchall()
