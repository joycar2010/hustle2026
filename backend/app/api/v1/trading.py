...
async def reconcile_open_ledger_spreads(user_accs, pair_code, db):
    """事后用历史配对(_build_paired_trades)的真实对冲成交价回填 pos_open_ledger.s,
    使 平均点差 与 /trading 价差 同源一致。账本空则廉价早退;LSET 定点改, 不删表。今日范围。"""
    from app.core.redis_client import redis_client as _rc
    import json as _json
    if not user_accs:
        return
    uid = str(user_accs[0].user_id)
    keys = {d: f'pos_open_ledger:{uid}:{pair_code}:{d}' for d in ('reverse', 'forward')}
    ledger = {}
    any_rows = False
    for _d, _k in keys.items():
        try:
            _rows = await _rc.client.lrange(_k, 0, -1)
        except Exception:
            _rows = []
        ledger[_d] = _rows or []
        if _rows:
    ...
    if not any_rows:
        return
    from app.services.hedging_pair_service import hedging_pair_service
    from app.utils.time_utils import beijing_to_utc_ms
    from datetime import datetime as _dt2, timedelta as _td2, timezone as _tz2
    pair = hedging_pair_service.get_pair(pair_code)
    sym_a = pair.symbol_a.symbol if pair else 'XAUUSDT'
    sym_b = pair.symbol_b.symbol if pair else 'XAUUSD+'
    a_pid = pair.symbol_a.platform_id if pair else 1
    b_pid = pair.symbol_b.platform_id if pair else 2
    conv = float(getattr(pair, 'conversion_factor', 100.0)) if pair else 100.0
    _now = _dt2.now(_tz2.utc); _bj = _now + _td2(hours=8)
    start_ms = beijing_to_utc_ms(_bj.strftime('%Y-%m-%d 00:00:00'))
    end_ms = int(_now.timestamp() * 1000)
    primary = [a for a in user_accs if (not a.is_mt5_account) and a.platform_id == a_pid]
    mt5a = [a for a in user_accs if a.is_mt5_account and a.platform_id == b_pid]
    if not primary:
        return
    btr = []
    for acc in primary:
        try:
            raw = await _get_binance_trades_realtime(acc, start_ms, end_ms, symbol=sym_a)
            btr.extend([(t, acc.account_name) for t in (raw or [])])
        except Exception:
            pass
    mtr = []
    for acc in mt5a:
        try:
            _t = await _get_mt5_trades_realtime(acc, start_ms, end_ms, db, mt5_symbol=sym_b)
            if _t:
                mtr.extend(_t)
        except Exception:
            pass
    fb = _format_binance_trades(btr)
    fm = _format_mt5_trades(mtr)
    paired = _build_paired_trades(fb, fm, conversion_factor=conv)
    oid2pair = {}
    for p in paired:
        ah = p.get('avg_price_hedge'); ap = p.get('avg_price_primary')
        if ah is None or ap is None:
            continue
        oid2pair[str(p.get('id'))] = (float(ap), float(ah))
    if not oid2pair:
        return
    for _d in ('reverse', 'forward'):
        rows = ledger.get(_d) or []
        for idx, raw in enumerate(rows):
            try:
                ...
                continue
            oid = str(e.get('oid'))
            if oid not in oid2pair:
                continue
            ap, ah = oid2pair[oid]
            real = round((ap - ah) if _d == 'reverse' else (ah - ap), 4)
            if abs(float(e.get('s', 0) or 0) - real) > 1e-4:
                e['s'] = real
                try:
                    await _rc.client.lset(keys[_d], idx, _json.dumps(e))
                except Exception:
                    pass


async def compute_daily_net_profit(user_accs, pair_code, db):
    """当日(北京00:00->now)返佣后净利润, 复用历史口径(_calculate_stats)。供 PnlFastStreamer。
    netProfitAfterRebate 仅依赖 binance已实现/资金费/返佣 + mt5成交, 故 binance_trades 传 []。"""
    from app.utils.time_utils import beijing_to_utc_ms
    from app.services.hedging_pair_service import hedging_pair_service
    from datetime import datetime as _dt2, timedelta as _td2, timezone as _tz2
    pair = hedging_pair_service.get_pair(pair_code)
    ...
    _now_bj = _now_utc + _td2(hours=8)
    start_ms = beijing_to_utc_ms(_now_bj.strftime('%Y-%m-%d 00:00:00'))
    end_ms = int(_now_utc.timestamp() * 1000)
    b_realized = 0.0; b_funding = 0.0; b_rebate = 0.0; mt5_raw = []
    for acc in user_accs:
        try:
            if (not acc.is_mt5_account) and acc.platform_id == a_pid:
                b_realized += await _get_binance_realized_pnl(acc, start_ms, end_ms, symbol=sym_a)
                _ft, _ = await _get_binance_funding_fee(acc, start_ms, end_ms, symbol=sym_a)
                b_funding += _ft
                b_rebate += await _get_binance_rebate(acc, start_ms, end_ms, symbol=sym_a)
            elif acc.is_mt5_account and acc.platform_id == b_pid:
                _tr = await _get_mt5_trades_realtime(acc, start_ms, end_ms, db, mt5_symbol=sym_b)
                if _tr:
                    mt5_raw.extend(_tr)
        except Exception as _e:
            logger.debug(f"[daily_net] acct {getattr(acc,'account_name','?')} skip: {_e}")
    formatted_mt5 = _format_mt5_trades(mt5_raw)
    stats = _calculate_stats([], formatted_mt5, b_realized, b_funding, binance_rebate=b_rebate)
    return float(stats.get('netProfitAfterRebate', 0.0))


@router.get("/open-entry-spread")
async def get_open_entry_spread(
    pair_code: str = Query("XAU"),
    strategy_type: str = Query(..., description="reverse | forward"),
    user_id: str = Depends(get_current_user_id),
):
    """当前持仓权威平均入场点差 + 在仓逐笔开仓明细(回灌,免疫刷新);源 Redis pos_open_ledger, 执行器FIFO维护。"""
    _dir = 'reverse' if 'reverse' in (strategy_type or '') else 'forward'
    key = f"pos_open_ledger:{user_id}:{pair_code}:{_dir}"
    avg = None
    total_qty = 0.0
    ladders: dict = {}
    try:
        from app.core.redis_client import redis_client as _rc
        import json as _json
        rows = await _rc.client.lrange(key, 0, -1)
        tot_w = 0.0
        for raw in (rows or []):
            try:
                e = _json.loads(raw)
            ...
            s = float(e.get('s') or 0)
            if q <= 0:
                continue
            total_qty += q
            tot_w += q * s
            idx = str(e.get('ladder', 0))
            ladders.setdefault(idx, {"trades": []})
            ladders[idx]["trades"].append({
                "binanceFilled": round(q, 4),
                "mt5Filled": round(float(e.get('m') or 0), 4),
                "spreadAtExecution": round(s, 4),
                "timestamp": e.get('ts'),
            })
        if total_qty > 0:
            avg = round(tot_w / total_qty, 4)
    except Exception as _e:
        logger.warning(f"[open-entry-spread] read failed: {_e}")
    return {
        "avg_entry_spread": avg,
...
def _build_paired_trades(binance_trades, mt5_trades, conversion_factor: float = 100.0):
    ...
    for oid, g in sorted(groups.items(), key=lambda kv: _grp_min_ts(kv[1])):
        ...
        if avg_price_h is not None:
            ...
            # 滑点 = 实得相对"设定阈值(threshold)"的偏离, 更有利为负(开/平符号化):
            #   开仓(求高价差): threshold - realized; 平仓(求低价差): realized - threshold
            # 与实时滑点护栏(actual_spread - threshold)同口径, 且可与表内"价差/阈值"两列对账。
            _thr = g.get("threshold")
            if g["source"] == "strategy" and _thr is not None and ("forward" in _st or "reverse" in _st):
                if "opening" in _st:
                    slippage = round(float(_thr) - realized, 4)
                else:
                    slippage = round(realized - float(_thr), 4)
            # 审计列: 相对"触发瞬时真实价差(trigger_spread)"的纯执行滑点(独立, 不替代上面的"滑点")
            _trig = g.get("trigger_spread")
            if g["source"] == "strategy" and _trig is not None and ("forward" in _st or "reverse" in _st):
                if "opening" in _st:
                    slippage_vs_trigger = round(float(_trig) - realized, 4)
                else:
                    slippage_vs_trigger = round(realized - float(_trig), 4)
        ...
        paired.append({
            ...
            "spread": spread,
            "slippage": slippage,
            "trigger_spread": g.get("trigger_spread"),
            "slippage_vs_trigger": slippage_vs_trigger,
