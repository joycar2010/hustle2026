...
class BybitV5Client:
    ...
    async def _request(
        ...
        **kwargs,
        ...
        # 需要将认证拆分为 proxy_url + proxy_auth 分别传入。
        raw_proxy = self.proxy_url if self.proxy_url and self.proxy_url != 'direct' else (settings.HTTPS_PROXY or settings.HTTP_PROXY or None)
        proxy = None
        proxy_auth = None
        if raw_proxy:
