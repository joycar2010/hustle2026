...
TERMINAL_ERROR_CODES = {
    ...
    # -5022 removed: GTX Post-Only rejection is transient (price moved during flight).
    # The continuous_executor will retry on the next trigger cycle with fresh prices.
    ...
    # Account compliance errors — user must fix these on Binance side
    -4411,   # Please sign TradFi-Perps agreement contract fapi
    -4412,   # Please upgrade to new account type
    -4400,   # API account not allowed to trade
    -4401,   # Account blocked for futures
    -4402,   # Account blocked for margin
    -4410,   # Intermediate verification required
    -4413,   # KYC verification required for large orders
    -2019,   # Margin is insufficient
    -2018,   # Balance is insufficient
    -4046,   # No need to change position side
    -4047,   # Invalid margin asset
...
class BinanceFuturesClient:
    ...
    def _split_proxy(self, raw_proxy: Optional[str]):
        """拆分代理 URL 为 (proxy_url, proxy_auth)

        aiohttp 不支持在 proxy URL 中内嵌认证（user:pass@host:port），
        需要将认证信息拆分为 proxy_auth=aiohttp.BasicAuth(...) 单独传入。
        """
        if not raw_proxy or raw_proxy == 'direct':
            return None, None
        if raw_proxy.startswith(('socks5://', 'socks4://', 'socks://')):
            return raw_proxy, None  # SOCKS 代理由 connector 处理
        from urllib.parse import urlparse
        parsed = urlparse(raw_proxy)
        if parsed.username and parsed.password:
            proxy_url = f"{parsed.scheme}://{parsed.hostname}:{parsed.port}"
            return proxy_url, aiohttp.BasicAuth(parsed.username, parsed.password)
    ...
    async def _request(
        ...
        **kwargs,
        ...
        if signed:
            ...
            try:
                # SOCKS5 代理由 ProxyConnector 处理；HTTP 代理用 proxy+proxy_auth 参数
                if getattr(self, '_proxy_via_connector', False):
                    proxy, proxy_auth = None, None
                else:
                    proxy, proxy_auth = self._split_proxy(self.proxy_url)
                async with session.request(method, full_url, headers=headers, proxy=proxy, proxy_auth=proxy_auth, **kwargs) as resp:
        ...
        try:
            ...
            if getattr(self, '_proxy_via_connector', False):
                ...
                proxy, proxy_auth = self._split_proxy(self.proxy_url)
            async with session.request(method, url, headers=headers, proxy=proxy, proxy_auth=proxy_auth, **kwargs) as resp:
                data = await resp.json()
    ...
    async def create_futures_listen_key(self) -> str:
        ...
        Requires X-MBX-APIKEY header but no HMAC signature.
        """
        session = await self._get_session()
        url = f"{self.base_url}/fapi/v1/listenKey"
        headers = {"X-MBX-APIKEY": self.api_key}
        proxy, proxy_auth = self._split_proxy(self.proxy_url if self.proxy_url and self.proxy_url != 'direct' and not getattr(self, '_proxy_via_connector', False) else None)
        async with session.request("POST", url, headers=headers, proxy=proxy, proxy_auth=proxy_auth) as resp:
            data = await resp.json()
            if resp.status != 200:
                self._raise_typed_error(data)
    ...
    async def keepalive_futures_listen_key(self, listen_key: str) -> None:
        ...
        proxy, proxy_auth = self._split_proxy(self.proxy_url if self.proxy_url and self.proxy_url != 'direct' and not getattr(self, '_proxy_via_connector', False) else None)
        async with session.request("PUT", url, headers=headers, params=params, proxy=proxy, proxy_auth=proxy_auth) as resp:
            if resp.status != 200:
                data = await resp.json()
                self._raise_typed_error(data)
    ...
    async def place_maker_order(
        ...
        Binance Futures natively supports server-side BBO matching via priceMatch:
          priceMatch=QUEUE  → price is set by Binance to the best bid (BUY) or
                              best ask (SELL) at the moment of matching, atomically.
          timeInForce=GTX   → Post-Only: rejected if the order would be TAKER.

        No client-side price calculation or +/- tick hacks needed.
        No race condition between quote read and order arrival.
        The 'price' parameter MUST NOT be sent when priceMatch is used.

        priceMatch values:
          QUEUE       — own-side BBO (best bid for BUY, best ask for SELL) → MAKER
          QUEUE_5/10  — 5/10 ticks behind own-side BBO (deeper MAKER)
          OPPONENT    — opposing-side BBO → immediate TAKER (not for MAKER use)
        """
        params = {
            "symbol": symbol,
            "side": side.upper(),
            ...
            "quantity": quantity,
            "priceMatch": "QUEUE",    # native MAKER: server-side BBO match
            "timeInForce": "GTX",     # Post-Only enforcement
        }
