...
async def get_aggregated_dashboard(
    ...
    try:
        ...
        # 不抛 500，而是回退到展示账户基础信息（带错误标记）
        logger.error(f"API: get_aggregated_account_data failed: {e}")
        return {
            "summary": {
                "total_assets": 0, "available_balance": 0, "net_assets": 0,
                "frozen_assets": 0, "margin_balance": 0, "unrealized_pnl": 0,
                "daily_pnl": 0, "risk_ratio": None,
                "account_count": len(active_accounts), "position_count": 0,
            },
            "accounts": [],
            "positions": [],
            "failed_accounts": [
