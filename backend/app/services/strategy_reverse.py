"""Reverse arbitrage strategy (long Bybit, short Binance)"""
...
class ReverseArbitrageStrategy(BaseStrategy):
    """Reverse arbitrage strategy: Buy Bybit, Sell Binance

    Entry condition: binance_ask - bybit_bid >= target_spread (negative spread)
    Entry execution: Sell Binance (ask - 0.01), Buy Bybit (bid + 0.01)

    Exit condition: bybit_ask - binance_bid <= close_spread
    Exit execution: Sell Bybit (ask - 0.01), Buy Binance (bid + 0.01)
    """

    def __init__(self, config: StrategyConfig):
        super().__init__(config)
        self.strategy_type = "reverse"
    ...
    async def execute_entry(
        ...
        """Execute reverse arbitrage entry
        ...
        if result["success"]:
            await self.send_notification(
                self.config.user_id,
                f"Reverse arbitrage entry executed: Sell Binance @ {binance_sell_price}, Buy Bybit @ {bybit_buy_price}",
                "success",
            )
        else:
            await self.send_notification(
                self.config.user_id,
                f"Reverse arbitrage entry failed: {result.get('error', 'Unknown error')}",
                "error",
            )
    ...
    async def execute_exit(
        ...
        """Execute reverse arbitrage exit
        ...
        if result["success"]:
            await self.send_notification(
                self.config.user_id,
                f"Reverse arbitrage exit executed: Buy Binance @ {binance_buy_price}, Sell Bybit @ {bybit_sell_price}",
                "success",
            )
        else:
            await self.send_notification(
                self.config.user_id,
                f"Reverse arbitrage exit failed: {result.get('error', 'Unknown error')}",
                "error",
            )
