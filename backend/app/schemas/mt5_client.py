...
class MT5ClientCreate(MT5ClientBase):
    """创建MT5客户端"""
    bridge_url: Optional[str] = Field(None, max_length=500, description="Bridge微服务地址")
    is_system_service: bool = Field(default=False, description="是否为系统服务账号")


class MT5ClientUpdate(BaseModel):
    """更新MT5客户端"""
    account_id: Optional[UUID] = None  # Admin only: transfer to another account
...
class MT5ClientResponse(MT5ClientBase):
    ...
    bridge_service_port: Optional[int] = None
    ...
    # Extra fields joined from Account / User / Platform
