...
from coincore.models import GlobalRules, FundRules, Blacklist
...
class GlobalRulesSnapshot:
    ...
    hedge_auto_converge: bool = False   # 净敞口自动收敛: 裸多自动 reduceOnly 对齐(默认关,只告警)
    spot_order_mode: str = "market"     # 现货腿下单: market市价/maker post-only限价
...
_SYSTEM_FIELDS = (
    ...
    "hedge_auto_converge", "spot_order_mode",
)
        ...
        try:
            ...
            if rules:
                self.global_rules = GlobalRulesSnapshot(
                    ...
                    hedge_auto_converge=bool(getattr(rules, "hedge_auto_converge", False)),
                    spot_order_mode=(getattr(rules, "spot_order_mode", None) or "market"),
                )
