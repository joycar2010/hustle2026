...
struct Hub {
    tx: broadcast::Sender<Arc<RoutedFrame>>,
    ...
    while let Some(msg) = stream.next().await {
        if let Ok(payload) = msg.get_payload::<String>() {
            match parse_routed_frame(payload) {
                Ok(frame) => {
                    let _ = hub.tx.send(Arc::new(frame));
                }
...
async fn handle_conn(hub: Arc<Hub>, stream: tokio::net::TcpStream, redis_url: String, dsn: String) {
    ...
    let mut pump = tokio::spawn(async move {
        while let Some((frame, skipped)) = next_frame_for_user(&mut rx, &uname).await {
            if skipped > 0 {
                tracing::warn!(client_id=id, user=%uname, skipped,
