...
const selectAccount = `SELECT account_id::text, user_id::text, platform_id, account_name,
	...
	is_mt5_account, is_default, is_active, leverage, proxy_config, create_time, update_time
	FROM accounts`
...
func scanAccount(row interface{ Scan(...any) error }) (*accountRow, error) {
	...
	return a, row.Scan(
		...
		&a.IsMT5Account, &a.IsDefault, &a.IsActive, &a.Leverage, &a.ProxyConfig,
...
func UpdateAccount(c *gin.Context) {
	...
	if body.ProxyConfig != nil {
		db.Pool().Exec(ctx, `UPDATE accounts SET proxy_config=$1, update_time=NOW() WHERE account_id=$2::uuid`, []byte(*body.ProxyConfig), accountID)
	}
...
func DeleteAccount(c *gin.Context) {
	...
	if isAdminUser(ctx, userID) {
		tag, err = db.Pool().Exec(ctx,
			`DELETE FROM accounts WHERE account_id=$1::uuid`, accountID)
	} else {
		tag, err = db.Pool().Exec(ctx,
